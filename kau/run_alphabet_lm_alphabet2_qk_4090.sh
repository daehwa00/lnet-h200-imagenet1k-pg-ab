...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_alphabet2_qk/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid QK execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_alphabet2_qk/generate_contract.py --check

readonly OUTPUT_ROOT="/home/daehwa/alphabet-lm-4090-qk-runs/${KAU_EXPECTED_COMMIT:0:16}"
...
[[ -f "${TRAIN_MANIFEST}" && -f "${VALIDATION_MANIFEST}" && -f "${TOKEN_Q_10M}" ]] \
  || { echo "ERROR: QK inputs are missing" >&2; exit 2; }
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: QK queue is already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-QK-Addressed-S501-v1"
...
run_stage() {
  ...
  timeout --signal=TERM --kill-after=5m 4h \
    ...
    --slow-cnn-pole-key \
    --slow-cnn-pole-key-rho 0.5 \
    --initialize-slow-key-trunk-checkpoint "${TOKEN_Q_10M}" \
    "${resume_args[@]}" \
...
run_stage alphabet2-qk-1m none 1000000 "${OUTPUT_ROOT}/qk-1m"
run_stage alphabet2-qk-2m "${OUTPUT_ROOT}/qk-1m/checkpoint.pt" 2000000 "${OUTPUT_ROOT}/qk-2m"
run_stage alphabet2-qk-4m "${OUTPUT_ROOT}/qk-2m/checkpoint.pt" 4000000 "${OUTPUT_ROOT}/qk-4m"
echo "KAU_ALPHABET_LM_QK_COMPLETE=${OUTPUT_ROOT}"
