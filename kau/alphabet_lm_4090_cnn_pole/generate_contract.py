#!/usr/bin/env python3
"""Generate the paired repeated CNN-pole campaign runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_cnn_pole/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_cnn_pole/campaign.runtime.json"
RECURRENT = "alphabet-cnn-pole-p128-6bank-recurrent"
CONTROL = "alphabet-cnn-pole-p128-6bank-no-recurrence"
...
def _render() -> str:
    ...
    if (
        manifest.get("schema") != "lnet.kau.alphabet_lm.cnn_pole_1m.v1"
        or manifest["training"]["execution"] != [RECURRENT, CONTROL]
        or manifest["training"]["target_tokens"] != 1_000_000
        or variants[RECURRENT]["cnn_pole_use_recurrence"] is not True
        or variants[CONTROL]["cnn_pole_use_recurrence"] is not False
        or any(variants[label]["cnn_pole_interval"] != 2 for label in variants)
        or any(variants[label]["cnn_pole_modes"] != 128 for label in variants)
        or set(manifest["parameter_counts"]) != {RECURRENT, CONTROL}
    ):
        raise RuntimeError("invalid repeated CNN-pole campaign")
    campaign_id = manifest["campaign_id"]

    def run(label: str, suffix: str) -> dict[str, object]:
        return {
            "id": hashlib.sha256(f"{campaign_id}\0{label}:seed501".encode()).hexdigest()[:16],
            "display_name": f"RTX4090-S501-ALPHABET-CNNPoleP128-6Bank-{suffix}-1M",
            "tags": [
                "RTX4090",
                "FineWeb-Edu",
                "CNNPole",
    ...
    runtime = {
        "schema": "lnet.kau.alphabet_lm.cnn_pole_1m.runtime.v1",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale repeated CNN-pole runtime", file=sys.stderr)
            return 1
        return 0
