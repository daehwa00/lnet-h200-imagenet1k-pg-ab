...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_content_rank_sweep_30m/campaign.runtime.json"
readonly PYTHON="${KAU_ALPHABET_PYTHON:-/home/daehwa/lnet-cffn-benchmark-20260808T091010Z/.venv/bin/python3}"
readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-content-rank-sweep-30m-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid content-rank sweep execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_content_rank_sweep_30m/generate_contract.py --check
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: content-rank sweep already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-ContentRankSweep-30M-S501-v1"
export WANDB_CONSOLE=off

run_one() {
  local rank="$1"
  local label="content-aligned-j${rank}-p32r16-l19-fromscratch-30m"
  timeout --signal=TERM --kill-after=5m 4h \
    ...
    --laplace-mamba-content-rank "${rank}" \
  ...
  timeout --signal=TERM --kill-after=3m 30m \
    ...
    --kind "alphabet2_content_aligned_j${rank}" \
...
echo "KAU_CONTENT_RANK_SWEEP_30M_COMPLETE=${OUTPUT_ROOT}"
