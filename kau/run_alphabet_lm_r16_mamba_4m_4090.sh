...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_r16_mamba_4m/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid R16/Mamba execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_r16_mamba_4m/generate_contract.py --check

readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-r16-mamba-4m-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
[[ -f "${TRAIN_MANIFEST}" && -f "${VALIDATION_MANIFEST}" ]] \
  || { echo "ERROR: paired 4M data inputs are missing" >&2; exit 2; }
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: paired 4M queue is already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-R16-vs-Mamba-4M-S501-v1"
...
timeout --signal=TERM --kill-after=5m 4h \
  ...
  --model alphabet \
  --run-label alphabet2-complex-vector-r16-e2e-4m \
  ...
  "${common[@]}" \
  --root "${OUTPUT_ROOT}/r16-e2e-4m"

timeout --signal=TERM --kill-after=5m 4h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model mamba \
  --run-label mamba-standard-e2e-4m \
  "${common[@]}" \
  --root "${OUTPUT_ROOT}/mamba-e2e-4m"

timeout --signal=TERM --kill-after=2m 30m \
  "${PYTHON}" scripts/evaluate_kau_alphabet_lm_context.py \
  --kind alphabet2_complex_vector_r16 \
  --checkpoint "${OUTPUT_ROOT}/r16-e2e-4m/checkpoint.pt" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --sequence-limit 512 \
  --output "${OUTPUT_ROOT}/r16-e2e-4m/context.json"

echo "KAU_ALPHABET_LM_R16_MAMBA_4M_COMPLETE=${OUTPUT_ROOT}"
