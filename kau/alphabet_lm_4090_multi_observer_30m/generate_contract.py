#!/usr/bin/env python3
"""Generate the Dense-Write Multi-Observer ALPHABET-2 30M runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_multi_observer_30m/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_multi_observer_30m/campaign.runtime.json"


def _render() -> str:
    raw = MANIFEST.read_bytes()
    manifest = json.loads(raw)
    label = "dense-write-multi-observer-j8-p32r16-l19-fromscratch-30m"
    if (
        manifest["training"]["execution"] != [label]
        or manifest["training"]["target_tokens"] != 30_000_000
        or manifest["architecture"]["observers"] != 8
        or manifest["parameter_counts"][label] != 64_105_427
        or not manifest["diagnostics"]["final_validation_only"]
    ):
        raise RuntimeError("invalid multi-observer campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-Dense-Write-Multi-Observer-J8-P32R16-L19-30M",
                "tags": [
                    "RTX4090",
                    "ALPHABET2",
                    "DenseWrite",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale multi-observer runtime", file=sys.stderr)
            return 1
        return 0
