...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_pole_reader_30m/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid pole-reader 30M execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_pole_reader_30m/generate_contract.py --check

readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-pole-reader-30m-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
readonly SOURCE="/home/daehwa/alphabet-lm-4090-pole-reader-runs/22123be3b6e90bd3/pole-reader-4m/checkpoint.pt"
readonly EXPECTED_SHA="6af20486c94447f541650fe30f202ef2ec094c385377a7a0def185e4273ac443"
[[ "$(sha256sum "${SOURCE}" | cut -d' ' -f1)" == "${EXPECTED_SHA}" ]] \
  || { echo "ERROR: pole-reader 4M source changed" >&2; exit 2; }
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: pole-reader 30M queue already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-PoleReaderK3R16-30M-S501-v1"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=5m 1h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model alphabet \
  --run-label pole-reader-k3-r16-e2e-30m \
  ...
  --root "${OUTPUT_ROOT}/pole-reader-30m"

timeout --signal=TERM --kill-after=2m 30m \
  "${PYTHON}" scripts/evaluate_kau_alphabet_lm_context.py \
  --kind alphabet2_pole_reader_r16 \
  --checkpoint "${OUTPUT_ROOT}/pole-reader-30m/checkpoint.pt" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --sequence-limit 512 \
  --output "${OUTPUT_ROOT}/pole-reader-30m/context.json"

echo "KAU_ALPHABET_LM_POLE_READER_30M_COMPLETE=${OUTPUT_ROOT}"
