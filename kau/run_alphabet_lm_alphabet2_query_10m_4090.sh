...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_alphabet2_query_10m/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid Token-Q 10M commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_alphabet2_query_10m/generate_contract.py --check

readonly OUTPUT_ROOT="/home/daehwa/alphabet-lm-4090-token-q-10m-runs/${KAU_EXPECTED_COMMIT:0:16}"
...
readonly TOKEN_Q_4M="/home/daehwa/alphabet-lm-4090-token-q-extension-runs/43a4f5dd69258c59/token-q-4m/checkpoint.pt"
[[ -f "${TRAIN_MANIFEST}" && -f "${VALIDATION_MANIFEST}" && -f "${V1_CHECKPOINT}" \
   && -f "${TOKEN_Q_4M}" ]] \
  || { echo "ERROR: Token-Q 10M inputs are missing" >&2; exit 2; }
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: Token-Q 10M queue is already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-TokenQ-10M-S501-v1"
export WANDB_CONSOLE=off

readonly RUN_ROOT="${OUTPUT_ROOT}/token-q-10m"
timeout --signal=TERM --kill-after=5m 4h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model alphabet \
  --run-label alphabet2-token-q-10m \
  ...
  --resume-extension-checkpoint "${TOKEN_Q_4M}" \
  --target-tokens-override 10000000 \
  --runtime "${RUNTIME}" \
...
echo "KAU_ALPHABET_LM_TOKEN_Q_10M_COMPLETE=${OUTPUT_ROOT}"
