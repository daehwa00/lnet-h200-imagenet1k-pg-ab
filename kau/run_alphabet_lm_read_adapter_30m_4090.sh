...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_read_adapter_30m/campaign.runtime.json"
readonly PYTHON="${KAU_ALPHABET_PYTHON:-/home/daehwa/lnet-cffn-benchmark-20260808T091010Z/.venv/bin/python3}"
readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-read-adapter-30m-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
readonly LABEL="dense-vector-read-adapter-p32r16-l19-fromscratch-30m"

cd "${PROJECT_ROOT}"
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  || [[ "$(git rev-parse HEAD)" != "${KAU_EXPECTED_COMMIT}" ]]; then
  echo "ERROR: invalid read-adapter execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_read_adapter_30m/generate_contract.py --check
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: read-adapter run already active" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-ReadAdapter-30M-S501-v1"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=5m 4h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model alphabet2_read_adapter_image_postfusion \
  --run-label "${LABEL}" \
...
timeout --signal=TERM --kill-after=3m 30m \
  ...
  --kind alphabet2_read_adapter_image_postfusion \
  --checkpoint "${OUTPUT_ROOT}/${LABEL}/checkpoint.pt" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --sequence-limit 128 \
  --output "${OUTPUT_ROOT}/${LABEL}/context.json"

echo "KAU_READ_ADAPTER_30M_COMPLETE=${OUTPUT_ROOT}"
