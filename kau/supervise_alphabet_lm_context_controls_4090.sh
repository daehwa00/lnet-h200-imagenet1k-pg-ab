...
while true; do
  ...
  if [[ "${processes}" != "${observed}" ]]; then
    printf 'CONTEXT_CONTROLS_WAITING_FOR_GPU=0 pids=%s time=%s\n' \
...
KAU_EXPECTED_COMMIT="${EXPECTED_COMMIT}" \
  "${PROJECT_ROOT}/kau/run_alphabet_lm_context_controls_4090.sh"
context_status=$?
set -e
printf 'CONTEXT_CONTROLS_EXIT=%s time=%s\n' \
  "${context_status}" "$(date --iso-8601=seconds)"
