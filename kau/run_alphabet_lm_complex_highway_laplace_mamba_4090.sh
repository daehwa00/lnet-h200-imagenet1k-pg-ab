...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_complex_highway_laplace_mamba/campaign.runtime.json"
readonly PYTHON="${KAU_ALPHABET_PYTHON:-/home/daehwa/lnet-cffn-benchmark-20260808T091010Z/.venv/bin/python3}"
readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-complex-highway-laplace-mamba-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
readonly LABEL="complex-highway-laplace-mamba-p32n4r16-l19-fromscratch-100m"

cd "${PROJECT_ROOT}"
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  || [[ "$(git rev-parse HEAD)" != "${KAU_EXPECTED_COMMIT}" ]]; then
  echo "ERROR: invalid Complex-Highway Laplace-Mamba execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_complex_highway_laplace_mamba/generate_contract.py --check
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: Complex-Highway Laplace-Mamba already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-ComplexHighwayLaplaceMamba-100M-S501-v1"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=5m 6h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model laplace_mamba_complex_highway \
  --run-label "${LABEL}" \
...
timeout --signal=TERM --kill-after=3m 30m \
  ...
  --kind laplace_mamba_complex_highway \
  --checkpoint "${OUTPUT_ROOT}/${LABEL}/checkpoint.pt" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --sequence-limit 128 \
  --output "${OUTPUT_ROOT}/${LABEL}/context.json"

echo "KAU_COMPLEX_HIGHWAY_LAPLACE_MAMBA_COMPLETE=${OUTPUT_ROOT}"
