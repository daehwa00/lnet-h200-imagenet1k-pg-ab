...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_slow_cnn_pole/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid slow CNN-pole execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_slow_cnn_pole/generate_contract.py --check

readonly OUTPUT_ROOT="/home/daehwa/alphabet-lm-4090-slow-cnn-pole-runs/${KAU_EXPECTED_COMMIT:0:16}"
...
[[ -f "${TRAIN_MANIFEST}" && -f "${VALIDATION_MANIFEST}" && -f "${TRUNK_CHECKPOINT}" ]] \
  || { echo "ERROR: slow CNN-pole inputs are missing" >&2; exit 2; }
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: slow CNN-pole queue is already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-CNN6-SlowP128-1M-S501-v1"
export WANDB_CONSOLE=off

for recurrence in recurrent no-recurrence; do
  if [[ "${recurrence}" == recurrent ]]; then
    label="alphabet-cnn6-slow-p128-recurrent"
    kind="cnn_pole_p128_6bank_slow_p128"
    recurrence_flag="--slow-cnn-pole-use-recurrence"
  else
    label="alphabet-cnn6-slow-p128-no-recurrence"
    kind="cnn_pole_p128_6bank_slow_p128_no_recurrence"
...
echo "KAU_ALPHABET_LM_SLOW_CNN_POLE_COMPLETE=${OUTPUT_ROOT}"
