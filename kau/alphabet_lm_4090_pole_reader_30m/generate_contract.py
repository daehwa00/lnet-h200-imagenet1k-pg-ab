#!/usr/bin/env python3
"""Generate the exact Dense-K3 pole-reader 4M-to-30M extension runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_pole_reader_30m/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_pole_reader_30m/campaign.runtime.json"
...
def _render() -> str:
    ...
    label = "pole-reader-k3-r16-e2e-30m"
    ...
    if (
        ...
        raise RuntimeError("invalid pole-reader 30M campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-ALPHABET2-PoleReaderK3R16-E2E-30M",
                "tags": ["RTX4090", "ALPHABET2", "PoleReader", "K3", "30M"],
            }
        },
    }
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale pole-reader 30M runtime", file=sys.stderr)
            return 1
        return 0
