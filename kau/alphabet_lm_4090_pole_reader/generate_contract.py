#!/usr/bin/env python3
"""Generate the pole-specific token-rate reader experiment."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_pole_reader/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_pole_reader/campaign.runtime.json"
...
def _render() -> str:
    ...
    if (
        ...
        raise RuntimeError("invalid pole-reader campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-ALPHABET2-PoleReaderK3R16-E2E-4M",
                "tags": ["RTX4090", "ALPHABET2", "PoleReader", "TokenRate", "4M"],
            }
        },
    }
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale pole-reader runtime", file=sys.stderr)
            return 1
        return 0
