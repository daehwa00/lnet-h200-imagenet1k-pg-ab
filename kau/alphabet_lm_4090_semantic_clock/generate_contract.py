#!/usr/bin/env python3
"""Generate the frozen Dense-K3 semantic-clock runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_semantic_clock/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_semantic_clock/campaign.runtime.json"
...
def _render() -> str:
    ...
    label = "semantic-clock-r16-frozen-4m"
    ...
    if (
        ...
        or not variant["slow_cnn_pole_semantic_clock"]
    ):
        raise RuntimeError("invalid semantic-clock campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-ALPHABET2-SemanticClockR16-Frozen-4M",
                "tags": [
                    "RTX4090",
                    "ALPHABET2",
                    "PoleReader",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale semantic-clock runtime", file=sys.stderr)
            return 1
        return 0
