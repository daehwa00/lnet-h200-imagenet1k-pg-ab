...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_dynamic_delta_30m/campaign.runtime.json"
readonly PYTHON="${KAU_ALPHABET_PYTHON:-/home/daehwa/lnet-cffn-benchmark-20260808T091010Z/.venv/bin/python3}"
readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-dynamic-delta-30m-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
readonly LABEL="dense-vector-exact-dynamic-delta-p32r16-l19-fromscratch-30m"

cd "${PROJECT_ROOT}"
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  || [[ "$(git rev-parse HEAD)" != "${KAU_EXPECTED_COMMIT}" ]]; then
  echo "ERROR: invalid dynamic-delta execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_dynamic_delta_30m/generate_contract.py --check
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: dynamic-delta run already active" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-DynamicDelta-30M-S501-v1"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=5m 4h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model alphabet2_dynamic_delta_image_postfusion \
  --run-label "${LABEL}" \
...
echo "KAU_DYNAMIC_DELTA_30M_COMPLETE=${OUTPUT_ROOT}"
