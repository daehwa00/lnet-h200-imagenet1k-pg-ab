...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_laplace_matrix_scaling/campaign.runtime.json"
readonly PYTHON="${KAU_ALPHABET_PYTHON:-/home/daehwa/lnet-cffn-benchmark-20260808T091010Z/.venv/bin/python3}"
readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-laplace-matrix-scaling-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
readonly SOURCE="/home/daehwa/alphabet-lm-4090-laplace-matrix-state-runs/cea739814d4c6007/laplace-matrix-p32n4r32-js4-4m/checkpoint.pt"
readonly EXPECTED_SHA="d2320e9312d4fc3a77dbd6dc01266bbd5ecaacad5b055d4b1351b6ef1b46578f"

cd "${PROJECT_ROOT}"
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  || [[ "$(git rev-parse HEAD)" != "${KAU_EXPECTED_COMMIT}" ]]; then
  echo "ERROR: invalid Laplace matrix scaling commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_laplace_matrix_scaling/generate_contract.py --check
[[ "$(sha256sum "${SOURCE}" | cut -d' ' -f1)" == "${EXPECTED_SHA}" ]] \
  || { echo "ERROR: Laplace matrix 4M source changed" >&2; exit 2; }
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: Laplace matrix scaling already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-LaplaceMatrixScaling-S501-v1"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=5m 4h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model alphabet \
  --run-label laplace-matrix-p32n4r32-js4-e2e-30m \
  ...
  --root "${OUTPUT_ROOT}/laplace-matrix-p32n4r32-js4-30m"
...
timeout --signal=TERM --kill-after=3m 30m \
  ...
  --checkpoint "${OUTPUT_ROOT}/laplace-matrix-p32n4r32-js4-30m/checkpoint.pt" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --sequence-limit 128 \
  --output "${OUTPUT_ROOT}/laplace-matrix-p32n4r32-js4-30m/context.json"

echo "KAU_ALPHABET_LM_LAPLACE_MATRIX_SCALING_COMPLETE=${OUTPUT_ROOT}"
