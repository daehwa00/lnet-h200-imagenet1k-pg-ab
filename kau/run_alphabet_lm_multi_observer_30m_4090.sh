...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_multi_observer_30m/campaign.runtime.json"
readonly PYTHON="${KAU_ALPHABET_PYTHON:-/home/daehwa/lnet-cffn-benchmark-20260808T091010Z/.venv/bin/python3}"
readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-multi-observer-30m-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
readonly LABEL="dense-write-multi-observer-j8-p32r16-l19-fromscratch-30m"

cd "${PROJECT_ROOT}"
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  || [[ "$(git rev-parse HEAD)" != "${KAU_EXPECTED_COMMIT}" ]]; then
  echo "ERROR: invalid multi-observer execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_multi_observer_30m/generate_contract.py --check
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: multi-observer run already active" >&2; exit 2; }
...
timeout --signal=TERM --kill-after=3m 30m \
  ...
  --kind alphabet2_multi_observer_image_postfusion \
  --checkpoint "${OUTPUT_ROOT}/${LABEL}/checkpoint.pt" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --sequence-limit 128 \
  --output "${OUTPUT_ROOT}/${LABEL}/context.json"

echo "KAU_MULTI_OBSERVER_30M_COMPLETE=${OUTPUT_ROOT}"
