#!/usr/bin/env python3
"""Generate the frozen Dense-K3 pole write-scheduler runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_pole_write_scheduler/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_pole_write_scheduler/campaign.runtime.json"
...
def _render() -> str:
    ...
    label = "pole-write-scheduler-r16-frozen-4m"
    ...
    if (
        ...
        raise RuntimeError("invalid pole write-scheduler campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-ALPHABET2-PoleWriteSchedulerR16-Frozen-4M",
                "tags": [
                    "RTX4090",
                    "ALPHABET2",
                    "PoleReader",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale pole write-scheduler runtime", file=sys.stderr)
            return 1
        return 0
