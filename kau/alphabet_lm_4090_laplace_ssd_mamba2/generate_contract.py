...
MANIFEST = ROOT / "kau/alphabet_lm_4090_laplace_ssd_mamba2/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_laplace_ssd_mamba2/campaign.runtime.json"
LABEL = "laplace-ssd-mamba2-p8-l18-fromscratch-30m"
...
def _render() -> str:
    ...
    if (
        ...
        or variant["laplace_mamba_layers"] != 18
        or variant["laplace_mamba_poles"] != 8
        or not variant["laplace_mamba_parallel_static_scan"]
        or variant["complex_state_per_layer"] != 8_192
        or manifest["parameter_counts"][LABEL] != 45_437_328
    ):
        raise RuntimeError("invalid Laplace-SSD Mamba-2 campaign")
    campaign_id = manifest["campaign_id"]
    runtime = {
        "schema": "lnet.kau.alphabet_lm.laplace_ssd_mamba2.runtime.v1",
        ...
        "runs": {
            LABEL: {
                ...
                "display_name": "RTX4090-S501-LaplaceSSD-Mamba2-P8-L18-30M",
                "tags": [
                    "RTX4090", "ALPHABET2", "Mamba2", "LaplaceSSD",
                    "AssociativeScan", "FromScratch", "30M"
                ],
            }
        },
    }
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale Laplace-SSD Mamba-2 runtime", file=sys.stderr)
            return 1
        return 0
