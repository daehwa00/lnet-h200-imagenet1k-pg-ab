...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_content_aligned_alphabet2/campaign.runtime.json"
readonly PYTHON="${KAU_ALPHABET_PYTHON:-/home/daehwa/lnet-cffn-benchmark-20260808T091010Z/.venv/bin/python3}"
readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-content-aligned-alphabet2-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
readonly LABEL="content-aligned-alphabet2-p32r16-l19-fromscratch-100m"

cd "${PROJECT_ROOT}"
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  || [[ "$(git rev-parse HEAD)" != "${KAU_EXPECTED_COMMIT}" ]]; then
  echo "ERROR: invalid content-aligned ALPHABET-2 execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_content_aligned_alphabet2/generate_contract.py --check
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: content-aligned ALPHABET-2 already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-ContentAlignedALPHABET2-100M-S501-v1"
...
timeout --signal=TERM --kill-after=3m 30m \
  ...
  --kind alphabet2_content_aligned_image_postfusion \
  --checkpoint "${OUTPUT_ROOT}/${LABEL}/checkpoint.pt" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --sequence-limit 128 \
  --output "${OUTPUT_ROOT}/${LABEL}/context.json"

echo "KAU_CONTENT_ALIGNED_ALPHABET2_COMPLETE=${OUTPUT_ROOT}"
