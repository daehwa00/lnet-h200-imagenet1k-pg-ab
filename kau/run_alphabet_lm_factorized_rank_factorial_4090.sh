...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_factorized_rank_factorial/campaign.runtime.json"
readonly PYTHON="${KAU_ALPHABET_PYTHON:-/home/daehwa/lnet-cffn-benchmark-20260808T091010Z/.venv/bin/python3}"
readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-factorized-rank-factorial-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid rank-factorial execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_factorized_rank_factorial/generate_contract.py --check
[[ "$(sha256sum "${SOURCE}" | cut -d' ' -f1)" == "${EXPECTED_SHA}" ]] \
  || { echo "ERROR: repeated P32R4 source changed" >&2; exit 2; }
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: factorized rank factorial already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-FactorizedRankFactorial-S501-v1"
...
run_variant() {
  ...
  timeout --signal=TERM --kill-after=5m 1h \
    ...
    --repeated-vector-pole-write-rank "${write_rank}" \
    --repeated-vector-pole-query-rank "${query_rank}" \
...
run_variant factorized-p32r32-je8-jq4-4m 8 4 alphabet2_factorized_vector_pole_p32r32_j8q4
run_variant factorized-p32r32-je4-jq8-4m 4 8 alphabet2_factorized_vector_pole_p32r32_j4q8
run_variant factorized-p32r32-je8-jq8-4m 8 8 alphabet2_factorized_vector_pole_p32r32_j8q8

echo "KAU_ALPHABET_LM_FACTORIZED_RANK_FACTORIAL_COMPLETE=${OUTPUT_ROOT}"
