#!/usr/bin/env python3
"""Generate the integrated Laplace-Mamba 100M runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_integrated_laplace_mamba/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_integrated_laplace_mamba/campaign.runtime.json"
...
def _render() -> str:
    ...
    label = "integrated-laplace-mamba-p32n4r16-l19-fromscratch-100m"
    ...
    if (
        ...
        raise RuntimeError("invalid integrated Laplace-Mamba campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-Integrated-Laplace-Mamba-P32N4R16-L19-100M",
                "tags": ["RTX4090", "LaplaceMamba", "FromScratch", "100M"],
            }
        },
    }
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale integrated Laplace-Mamba runtime", file=sys.stderr)
            return 1
        return 0
