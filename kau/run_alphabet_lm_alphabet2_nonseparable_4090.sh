...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_alphabet2_nonseparable/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid non-separable memory execution commit" >&2
  exit 2
fi
...
readonly MATRIX_K4V4_4M="/home/daehwa/alphabet-lm-4090-matrix-k4v4-runs/61d7b44d3d82207a/matrix-4m/checkpoint.pt"
[[ -f "${TRAIN_MANIFEST}" && -f "${VALIDATION_MANIFEST}" && -f "${MATRIX_K4V4_4M}" ]] \
  || { echo "ERROR: non-separable memory inputs are missing" >&2; exit 2; }
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: non-separable memory queue is already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-NonseparableK4V4-S501-v1"
...
run_stage() {
  ...
  timeout --signal=TERM --kill-after=5m 4h \
    ...
    --slow-cnn-pole-independent-matrix-value \
    --initialize-slow-independent-value-checkpoint "${MATRIX_K4V4_4M}" \
    "${resume_args[@]}" \
  ...
  if [[ ! -f "${root}/context.json" ]]; then
    timeout --signal=TERM --kill-after=2m 30m \
      ...
      --kind alphabet2_nonseparable_k4v4 \
...
run_stage alphabet2-nonseparable-k4v4-1m none 1000000 \
  "${OUTPUT_ROOT}/nonseparable-1m"
run_stage alphabet2-nonseparable-k4v4-2m \
  "${OUTPUT_ROOT}/nonseparable-1m/checkpoint.pt" 2000000 \
  "${OUTPUT_ROOT}/nonseparable-2m"
run_stage alphabet2-nonseparable-k4v4-4m \
  "${OUTPUT_ROOT}/nonseparable-2m/checkpoint.pt" 4000000 \
  "${OUTPUT_ROOT}/nonseparable-4m"
echo "KAU_ALPHABET_LM_NONSEPARABLE_K4V4_COMPLETE=${OUTPUT_ROOT}"
