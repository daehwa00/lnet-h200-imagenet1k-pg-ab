...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_semantic_clock/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid semantic-clock execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_semantic_clock/generate_contract.py --check

readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-semantic-clock-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
flock -n 9 || { echo "ERROR: semantic-clock queue already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-SemanticClockR16-4M-S501-v1"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=5m 1h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model alphabet \
  --run-label semantic-clock-r16-frozen-4m \
  ...
  --slow-cnn-pole-semantic-clock \
  --initialize-slow-semantic-clock-checkpoint "${SOURCE}" \
  --target-tokens-override 4000000 \
  --runtime "${RUNTIME}" \
  --train-manifest "${TRAIN_MANIFEST}" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --root "${OUTPUT_ROOT}/semantic-clock-4m"
...
timeout --signal=TERM --kill-after=2m 30m \
  ...
  --checkpoint "${OUTPUT_ROOT}/semantic-clock-4m/checkpoint.pt" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --sequence-limit 512 \
  --output "${OUTPUT_ROOT}/semantic-clock-4m/context.json"

echo "KAU_ALPHABET_LM_SEMANTIC_CLOCK_COMPLETE=${OUTPUT_ROOT}"
