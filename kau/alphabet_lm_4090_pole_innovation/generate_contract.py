#!/usr/bin/env python3
"""Generate the frozen Dense-K3 pole innovation runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_pole_innovation/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_pole_innovation/campaign.runtime.json"
...
def _render() -> str:
    ...
    label = "pole-innovation-k3-r16-frozen-4m"
    ...
    if (
        ...
        or variant["slow_cnn_pole_innovation_kernel"] != 3
        or not variant["slow_cnn_pole_specific_reader"]
        or not variant["slow_cnn_pole_innovation"]
    ):
        raise RuntimeError("invalid pole innovation campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-ALPHABET2-PoleInnovationK3R16-Frozen-4M",
                "tags": [
                    "RTX4090",
                    "ALPHABET2",
                    "PoleReader",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale pole innovation runtime", file=sys.stderr)
            return 1
        return 0
