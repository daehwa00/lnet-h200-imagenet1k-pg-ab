...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_tensorpole/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid TensorPole-M8 execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_tensorpole/generate_contract.py --check

readonly OUTPUT_ROOT="/home/daehwa/alphabet-lm-4090-tensorpole-runs/${KAU_EXPECTED_COMMIT:0:16}"
...
flock -n 9 || { echo "ERROR: TensorPole-M8 queue is already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-TensorPole-M8-2M-S501-v1"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=2m 30m \
  "${PYTHON}" scripts/smoke_kau_alphabet_lm_4090.py --only tensorpole

readonly LABEL="alphabet-dense-k3-tensorpole-m8"
...
timeout --signal=TERM --kill-after=5m 4h \
  ...
  --memory-layout tensor_product \
  --tensor-temporal-modes 8 \
  --tensor-initial-read-gain 0.6 \
...
echo "KAU_ALPHABET_LM_TENSORPOLE_COMPLETE=${OUTPUT_ROOT}"
