...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_alphabet2_complex_vector_r16/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid complex-vector R16 execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_alphabet2_complex_vector_r16/generate_contract.py --check

readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-complex-vector-r16-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
[[ -f "${TRAIN_MANIFEST}" && -f "${VALIDATION_MANIFEST}" && -f "${TOKEN_Q_10M}" ]] \
  || { echo "ERROR: complex-vector R16 inputs are missing" >&2; exit 2; }
readonly EXPECTED_SOURCE_SHA="$(${PYTHON} -c 'import json,sys; print(json.load(open(sys.argv[1]))["source"]["token_q_10m_sha256"])' "${RUNTIME}")"
readonly ACTUAL_SOURCE_SHA="$(sha256sum "${TOKEN_Q_10M}" | cut -d' ' -f1)"
...
flock -n 9 || { echo "ERROR: complex-vector R16 queue is already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-ComplexVectorR16-S501-v1"
...
run_stage() {
  ...
  timeout --signal=TERM --kill-after=5m 4h \
    ...
    --initialize-slow-full-complex-vector-checkpoint "${TOKEN_Q_10M}" \
    "${resume_args[@]}" \
  ...
  if [[ ! -f "${root}/context.json" ]]; then
    timeout --signal=TERM --kill-after=2m 30m \
      ...
      --kind alphabet2_complex_vector_r16 \
