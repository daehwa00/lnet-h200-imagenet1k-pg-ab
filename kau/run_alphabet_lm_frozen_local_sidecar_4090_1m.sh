...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_frozen_local_sidecar/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid frozen local-sidecar execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_frozen_local_sidecar/generate_contract.py --check

readonly OUTPUT_ROOT="/home/daehwa/alphabet-lm-4090-frozen-local-sidecar-runs/${KAU_EXPECTED_COMMIT:0:16}"
...
[[ -f "${TRAIN_MANIFEST}" && -f "${VALIDATION_MANIFEST}" && -f "${TRUNK_CHECKPOINT}" ]] \
  || { echo "ERROR: frozen local-sidecar inputs are missing" >&2; exit 2; }
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: frozen local-sidecar queue is already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-FrozenLocalSidecar-1M-S501-v1"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=2m 30m \
  "${PYTHON}" scripts/smoke_kau_alphabet_lm_4090.py --only normalized_local_sidecar
readonly RUN_ROOT="${OUTPUT_ROOT}/run"
timeout --signal=TERM --kill-after=5m 4h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model alphabet \
  --run-label alphabet-dense-k3-frozen-normalized-local-sidecar \
...
if [[ ! -f "${RUN_ROOT}/context.json" ]]; then
  timeout --signal=TERM --kill-after=2m 20m \
    ...
    --kind dense_local_sidecar_normalized_no_recurrence \
    --checkpoint "${RUN_ROOT}/checkpoint.pt" \
    --validation-manifest "${VALIDATION_MANIFEST}" \
    --sequence-limit 512 \
    --output "${RUN_ROOT}/context.json"
fi
echo "KAU_ALPHABET_LM_FROZEN_LOCAL_SIDECAR_COMPLETE=${OUTPUT_ROOT}"
