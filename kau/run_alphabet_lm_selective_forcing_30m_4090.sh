...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_selective_forcing_30m/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid selective-forcing execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_selective_forcing_30m/generate_contract.py --check

readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-selective-forcing-30m-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
flock -n 9 || { echo "ERROR: selective-forcing queue already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-SelectiveForcingR16-30M-S501-v2"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=5m 1h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model alphabet \
  --run-label selective-forcing-r16-e2e-30m \
  ...
  --root "${OUTPUT_ROOT}/selective-forcing-30m"
...
timeout --signal=TERM --kill-after=2m 30m \
  ...
  --checkpoint "${OUTPUT_ROOT}/selective-forcing-30m/checkpoint.pt" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --sequence-limit 512 \
  --output "${OUTPUT_ROOT}/selective-forcing-30m/context.json"

echo "KAU_ALPHABET_LM_SELECTIVE_FORCING_30M_COMPLETE=${OUTPUT_ROOT}"
