...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_alphabet2_matrix/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid matrix-memory execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_alphabet2_matrix/generate_contract.py --check

readonly OUTPUT_ROOT="/home/daehwa/alphabet-lm-4090-matrix-k4v4-runs/${KAU_EXPECTED_COMMIT:0:16}"
...
readonly VECTOR_D4_4M="/home/daehwa/alphabet-lm-4090-vector-d4-runs/0451b5d686be4f32/vector-4m/checkpoint.pt"
[[ -f "${TRAIN_MANIFEST}" && -f "${VALIDATION_MANIFEST}" && -f "${VECTOR_D4_4M}" ]] \
  || { echo "ERROR: matrix-memory inputs are missing" >&2; exit 2; }
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: matrix-memory queue is already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-MatrixK4V4-S501-v1"
...
run_stage() {
  ...
  timeout --signal=TERM --kill-after=5m 4h \
    ...
    --slow-cnn-pole-value-width 4 \
    --slow-cnn-pole-matrix-key-width 4 \
    --initialize-slow-matrix-trunk-checkpoint "${VECTOR_D4_4M}" \
    "${resume_args[@]}" \
...
run_stage alphabet2-matrix-k4v4-1m none 1000000 "${OUTPUT_ROOT}/matrix-1m"
run_stage alphabet2-matrix-k4v4-2m "${OUTPUT_ROOT}/matrix-1m/checkpoint.pt" 2000000 \
  "${OUTPUT_ROOT}/matrix-2m"
run_stage alphabet2-matrix-k4v4-4m "${OUTPUT_ROOT}/matrix-2m/checkpoint.pt" 4000000 \
  "${OUTPUT_ROOT}/matrix-4m"
echo "KAU_ALPHABET_LM_MATRIX_K4V4_COMPLETE=${OUTPUT_ROOT}"
