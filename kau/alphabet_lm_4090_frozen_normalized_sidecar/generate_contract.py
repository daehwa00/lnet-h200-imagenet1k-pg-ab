#!/usr/bin/env python3
"""Generate the frozen LocalOnly normalized-sidecar runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_frozen_normalized_sidecar/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_frozen_normalized_sidecar/campaign.runtime.json"
LABEL = "alphabet-dense-k3-frozen-normalized-sidecar"
...
def _render() -> str:
    ...
    if (
        manifest.get("schema") != "lnet.kau.alphabet_lm.frozen_normalized_sidecar_1m.v1"
        ...
        raise RuntimeError("invalid frozen normalized-sidecar campaign")
    campaign_id = manifest["campaign_id"]
    runtime = {
        "schema": "lnet.kau.alphabet_lm.frozen_normalized_sidecar_1m.runtime.v1",
        ...
        "runs": {
            LABEL: {
                ...
                "display_name": "RTX4090-S501-ALPHABET-FrozenLocal-NormSidecar-1M",
                "tags": [
                    "RTX4090",
                    "FineWeb-Edu",
                    "1M",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale frozen normalized-sidecar runtime", file=sys.stderr)
            return 1
        return 0
