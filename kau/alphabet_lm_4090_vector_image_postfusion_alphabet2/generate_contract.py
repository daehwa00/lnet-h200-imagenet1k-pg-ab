#!/usr/bin/env python3
"""Generate the VectorPole image-PostFusion ALPHABET-2 100M runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_vector_image_postfusion_alphabet2/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_vector_image_postfusion_alphabet2/campaign.runtime.json"
...
def _render() -> str:
    ...
    label = "vector-image-postfusion-alphabet2-p32r16-l19-fromscratch-100m"
    if (
        ...
        or manifest["parameter_counts"][label] != 44_308_947
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-Vector-Image-PostFusion-ALPHABET2-P32R16-L19-100M",
                "tags": [
                    "RTX4090",
                    "ALPHABET2",
                    "VectorPole",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale VectorPole image-PostFusion runtime", file=sys.stderr)
            return 1
        return 0
