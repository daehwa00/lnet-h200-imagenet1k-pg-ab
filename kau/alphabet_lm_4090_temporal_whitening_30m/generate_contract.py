#!/usr/bin/env python3
"""Generate the J2 pole-axis-whitening ALPHABET-2 30M runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_temporal_whitening_30m/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_temporal_whitening_30m/campaign.runtime.json"


def _render() -> str:
    raw = MANIFEST.read_bytes()
    manifest = json.loads(raw)
    label = "j2-pole-axis-whitening-p32r16-l19-fromscratch-30m"
    if (
        manifest["training"]["execution"] != [label]
        or manifest["training"]["target_tokens"] != 30_000_000
        or manifest["architecture"]["content_rank"] != 2
        or manifest["parameter_counts"][label] != 49_630_163
        or not manifest["diagnostics"]["final_validation_only"]
    ):
        raise RuntimeError("invalid temporal-whitening campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-J2-Pole-Axis-Whitening-P32R16-L19-30M",
                "tags": [
                    "RTX4090",
                    "ALPHABET2",
                    "ContentRank2",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale temporal-whitening runtime", file=sys.stderr)
            return 1
        return 0
