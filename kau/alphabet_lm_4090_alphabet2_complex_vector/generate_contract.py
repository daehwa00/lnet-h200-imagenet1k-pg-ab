#!/usr/bin/env python3
"""Generate complex-excitation VectorPole-R4 runtimes."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_alphabet2_complex_vector/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_alphabet2_complex_vector/campaign.runtime.json"
...
def _render() -> str:
    ...
    expected = [
        "alphabet2-complex-vector-r4-1m",
        "alphabet2-complex-vector-r4-2m",
        "alphabet2-complex-vector-r4-4m",
    ]
    if (
        ...
        raise RuntimeError("invalid complex-vector campaign")
    ...
    for label in execution:
        ...
        runs[label] = {
            ...
            "display_name": f"RTX4090-S501-ALPHABET2-ComplexVectorR4-{milestone}",
            "tags": ["RTX4090", "ALPHABET2", "ComplexVectorR4", milestone, "seed501"],
        }
    runtime = {
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale complex-vector runtime", file=sys.stderr)
            return 1
        return 0
