...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_pole_reader/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid pole-reader execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_pole_reader/generate_contract.py --check

readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-pole-reader-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
flock -n 9 || { echo "ERROR: pole-reader queue already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-PoleReaderK3R16-S501-v1"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=5m 1h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model alphabet \
  --run-label pole-reader-r16-e2e-4m \
  ...
  --root "${OUTPUT_ROOT}/pole-reader-4m"

timeout --signal=TERM --kill-after=2m 30m \
  "${PYTHON}" scripts/evaluate_kau_alphabet_lm_context.py \
  --kind alphabet2_pole_reader_r16 \
  --checkpoint "${OUTPUT_ROOT}/pole-reader-4m/checkpoint.pt" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --sequence-limit 512 \
  --output "${OUTPUT_ROOT}/pole-reader-4m/context.json"

echo "KAU_ALPHABET_LM_POLE_READER_COMPLETE=${OUTPUT_ROOT}"
