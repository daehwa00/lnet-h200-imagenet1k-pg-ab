#!/usr/bin/env python3
"""Generate the exact pole-wise dynamic-delta ALPHABET-2 30M runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_dynamic_delta_30m/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_dynamic_delta_30m/campaign.runtime.json"


def _render() -> str:
    raw = MANIFEST.read_bytes()
    manifest = json.loads(raw)
    label = "dense-vector-exact-dynamic-delta-p32r16-l19-fromscratch-30m"
    if (
        manifest["training"]["execution"] != [label]
        or manifest["training"]["target_tokens"] != 30_000_000
        or manifest["architecture"]["delta_hidden"] != 32
        or manifest["parameter_counts"][label] != 64_441_043
        or not manifest["diagnostics"]["final_validation_only"]
    ):
        raise RuntimeError("invalid dynamic-delta campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-Dense-Vector-Exact-Dynamic-Delta-P32R16-L19-30M",
                "tags": [
                    "RTX4090",
                    "ALPHABET2",
                    "DenseWrite",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale dynamic-delta runtime", file=sys.stderr)
            return 1
        return 0
