#!/usr/bin/env python3
"""Generate the repeated token-rate VectorPole 30M runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_repeated_vector_pole_30m/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_repeated_vector_pole_30m/campaign.runtime.json"
...
def _render() -> str:
    ...
    label = "repeated-vector-pole-p32r4-e2e-30m"
    ...
    if (
        ...
        or variant["repeated_vector_pole_interval"] != 1
        or variant["repeated_vector_pole_modes"] != 32
        or variant["repeated_vector_pole_width"] != 4
        or variant["repeated_vector_pole_reader_kernel"] != 3
        or not variant["repeated_vector_pole_memory"]
    ):
        raise RuntimeError("invalid repeated VectorPole 30M campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-ALPHABET2-RepeatedVectorPoleP32R4-E2E-30M",
                "tags": [
                    "RTX4090",
                    "ALPHABET2",
                    "RepeatedVectorPole",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale repeated VectorPole runtime", file=sys.stderr)
            return 1
        return 0
