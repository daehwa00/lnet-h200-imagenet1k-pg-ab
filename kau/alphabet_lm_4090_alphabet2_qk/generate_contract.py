#!/usr/bin/env python3
"""Generate mandatory-key QK addressing runtimes."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_alphabet2_qk/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_alphabet2_qk/campaign.runtime.json"
...
def _render() -> str:
    ...
    if (
        manifest.get("schema") != "lnet.kau.alphabet_lm.alphabet2_qk.v1"
        or execution != ["alphabet2-qk-1m", "alphabet2-qk-2m", "alphabet2-qk-4m"]
        ...
        or any(variant["slow_cnn_pole_key"] is not True for variant in variants.values())
    ):
        raise RuntimeError("invalid QK addressing campaign")
    ...
    for label in execution:
        ...
        runs[label] = {
            ...
            "display_name": f"RTX4090-S501-ALPHABET2-QK-{milestone}",
            "tags": ["RTX4090", "ALPHABET2", "QK", milestone, "seed501"],
        }
    runtime = {
        "schema": "lnet.kau.alphabet_lm.alphabet2_qk.runtime.v1",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale QK runtime", file=sys.stderr)
            return 1
        return 0
