#!/usr/bin/env python3
"""Generate the pole-wise read-adapter ALPHABET-2 30M runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_read_adapter_30m/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_read_adapter_30m/campaign.runtime.json"


def _render() -> str:
    raw = MANIFEST.read_bytes()
    manifest = json.loads(raw)
    label = "dense-vector-read-adapter-p32r16-l19-fromscratch-30m"
    if (
        manifest["training"]["execution"] != [label]
        or manifest["training"]["target_tokens"] != 30_000_000
        or manifest["parameter_counts"][label] != 64_416_723
        or not manifest["diagnostics"]["final_validation_only"]
    ):
        raise RuntimeError("invalid read-adapter campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-Dense-Vector-Read-Adapter-P32R16-L19-30M",
                "tags": [
                    "RTX4090",
                    "ALPHABET2",
                    "DenseWrite",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale read-adapter runtime", file=sys.stderr)
            return 1
        return 0
