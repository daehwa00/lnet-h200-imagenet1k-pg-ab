...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_alphabet2_vector/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid vector-memory execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_alphabet2_vector/generate_contract.py --check

readonly OUTPUT_ROOT="/home/daehwa/alphabet-lm-4090-vector-d4-runs/${KAU_EXPECTED_COMMIT:0:16}"
...
[[ -f "${TRAIN_MANIFEST}" && -f "${VALIDATION_MANIFEST}" && -f "${TOKEN_Q_10M}" ]] \
  || { echo "ERROR: vector-memory inputs are missing" >&2; exit 2; }
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: vector-memory queue is already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-VectorD4-S501-v1"
...
run_stage() {
  ...
  timeout --signal=TERM --kill-after=5m 4h \
    ...
    --initialize-slow-value-trunk-checkpoint "${TOKEN_Q_10M}" \
    "${resume_args[@]}" \
...
run_stage alphabet2-vector-d4-1m none 1000000 "${OUTPUT_ROOT}/vector-1m"
run_stage alphabet2-vector-d4-2m "${OUTPUT_ROOT}/vector-1m/checkpoint.pt" 2000000 \
  "${OUTPUT_ROOT}/vector-2m"
run_stage alphabet2-vector-d4-4m "${OUTPUT_ROOT}/vector-2m/checkpoint.pt" 4000000 \
  "${OUTPUT_ROOT}/vector-4m"
echo "KAU_ALPHABET_LM_VECTOR_D4_COMPLETE=${OUTPUT_ROOT}"
