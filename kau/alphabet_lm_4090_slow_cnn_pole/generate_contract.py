#!/usr/bin/env python3
"""Generate the paired slow CNN-pole campaign runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_slow_cnn_pole/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_slow_cnn_pole/campaign.runtime.json"
RECURRENT = "alphabet-cnn6-slow-p128-recurrent"
CONTROL = "alphabet-cnn6-slow-p128-no-recurrence"
...
def _render() -> str:
    ...
    if (
        manifest.get("schema") != "lnet.kau.alphabet_lm.slow_cnn_pole_1m.v1"
        or manifest["training"]["execution"] != [RECURRENT, CONTROL]
        or manifest["training"]["target_tokens"] != 1_000_000
        or variants[RECURRENT]["slow_cnn_pole_use_recurrence"] is not True
        or variants[CONTROL]["slow_cnn_pole_use_recurrence"] is not False
    ...
    runtime = {
        "schema": "lnet.kau.alphabet_lm.slow_cnn_pole_1m.runtime.v1",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale slow CNN-pole runtime", file=sys.stderr)
            return 1
        return 0
