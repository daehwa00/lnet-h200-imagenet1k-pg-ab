...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_integrated_laplace_mamba_bc_norm_bias/campaign.runtime.json"
readonly PYTHON="${KAU_ALPHABET_PYTHON:-/home/daehwa/lnet-cffn-benchmark-20260808T091010Z/.venv/bin/python3}"
readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-integrated-laplace-mamba-bc-norm-bias-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
readonly LABEL="integrated-laplace-mamba-bc-norm-bias-p32n4r16-l19-fromscratch-100m"

cd "${PROJECT_ROOT}"
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  || [[ "$(git rev-parse HEAD)" != "${KAU_EXPECTED_COMMIT}" ]]; then
  echo "ERROR: invalid integrated Laplace-Mamba B/C norm-bias execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_integrated_laplace_mamba_bc_norm_bias/generate_contract.py --check
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: integrated Laplace-Mamba B/C norm-bias already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-IntegratedLaplaceMamba-BCNormBias-100M-S501-v1"
...
timeout --signal=TERM --kill-after=5m 6h \
  ...
  --laplace-mamba-address-norm-bias \
...
echo "KAU_INTEGRATED_LAPLACE_MAMBA_BC_NORM_BIAS_COMPLETE=${OUTPUT_ROOT}"
