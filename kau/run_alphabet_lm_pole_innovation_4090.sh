...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_pole_innovation/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid pole innovation execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_pole_innovation/generate_contract.py --check

readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-pole-innovation-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
flock -n 9 || { echo "ERROR: pole innovation queue already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-PoleInnovationK3R16-4M-S501-v1"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=5m 1h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model alphabet \
  --run-label pole-innovation-k3-r16-frozen-4m \
  ...
  --slow-cnn-pole-innovation \
  --slow-cnn-pole-innovation-kernel 3 \
  --initialize-slow-innovation-checkpoint "${SOURCE}" \
  --target-tokens-override 4000000 \
  --runtime "${RUNTIME}" \
  --train-manifest "${TRAIN_MANIFEST}" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --root "${OUTPUT_ROOT}/innovation-4m"
...
timeout --signal=TERM --kill-after=2m 30m \
  ...
  --checkpoint "${OUTPUT_ROOT}/innovation-4m/checkpoint.pt" \
  --validation-manifest "${VALIDATION_MANIFEST}" \
  --sequence-limit 512 \
  --output "${OUTPUT_ROOT}/innovation-4m/context.json"

echo "KAU_ALPHABET_LM_POLE_INNOVATION_COMPLETE=${OUTPUT_ROOT}"
