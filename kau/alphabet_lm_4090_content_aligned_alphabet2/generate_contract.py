#!/usr/bin/env python3
"""Generate the content-aligned Vector ALPHABET-2 100M runtime."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_content_aligned_alphabet2/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_content_aligned_alphabet2/campaign.runtime.json"
...
def _render() -> str:
    ...
    label = "content-aligned-alphabet2-p32r16-l19-fromscratch-100m"
    if (
        ...
        or manifest["parameter_counts"][label] != 49_377_235
        or not manifest["diagnostics"]["final_validation_only"]
    ):
        raise RuntimeError("invalid content-aligned ALPHABET-2 campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-Content-Aligned-ALPHABET2-P32R16-L19-100M",
                "tags": [
                    "RTX4090",
                    "ALPHABET2",
                    "ContentAligned",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale content-aligned ALPHABET-2 runtime", file=sys.stderr)
            return 1
        return 0
