...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_grouped/campaign.runtime.json"
...
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  ...
  echo "ERROR: invalid grouped KAU execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_grouped/generate_contract.py --check

readonly OUTPUT_ROOT="/home/daehwa/alphabet-lm-4090-grouped-runs/${KAU_EXPECTED_COMMIT:0:16}"
...
flock -n 9 || { echo "ERROR: grouped KAU queue is already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-GroupedH8P128-10M-S501-v1"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=2m 30m \
  "${PYTHON}" scripts/smoke_kau_alphabet_lm_4090.py --only grouped

timeout --signal=TERM --kill-after=5m 8h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model alphabet \
  --run-label alphabet-grouped-h8p128 \
  --pole-initialization lifetime_palette \
  --memory-banks 8 \
  --bank-pole-modes 128 \
  --runtime "${RUNTIME}" \
...
echo "KAU_ALPHABET_LM_GROUPED_COMPLETE=${OUTPUT_ROOT}"
