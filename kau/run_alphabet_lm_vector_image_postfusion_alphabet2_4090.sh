...
readonly RUNTIME="${PROJECT_ROOT}/kau/alphabet_lm_4090_vector_image_postfusion_alphabet2/campaign.runtime.json"
readonly PYTHON="${KAU_ALPHABET_PYTHON:-/home/daehwa/lnet-cffn-benchmark-20260808T091010Z/.venv/bin/python3}"
readonly OUTPUT_ROOT="${KAU_OUTPUT_ROOT:-/home/daehwa/alphabet-lm-4090-vector-image-postfusion-alphabet2-runs/${KAU_EXPECTED_COMMIT:0:16}}"
...
readonly LABEL="vector-image-postfusion-alphabet2-p32r16-l19-fromscratch-100m"

cd "${PROJECT_ROOT}"
if [[ ! "${KAU_EXPECTED_COMMIT:-}" =~ ^[0-9a-f]{40}$ ]] \
  || [[ "$(git rev-parse HEAD)" != "${KAU_EXPECTED_COMMIT}" ]]; then
  echo "ERROR: invalid VectorPole image-PostFusion execution commit" >&2
  exit 2
fi
python3 kau/alphabet_lm_4090_vector_image_postfusion_alphabet2/generate_contract.py --check
mkdir -p "${OUTPUT_ROOT}"
exec 9>"${OUTPUT_ROOT}/queue.lock"
flock -n 9 || { echo "ERROR: VectorPole image-PostFusion already running" >&2; exit 2; }
...
export WANDB_GROUP="ALPHABET-LM-RTX4090-VectorImagePostFusionAlphabet2-100M-S501-v1"
export WANDB_CONSOLE=off

timeout --signal=TERM --kill-after=5m 8h \
  "${PYTHON}" scripts/train_h200_alphabet_lm_10m.py \
  --model alphabet2_vector_image_postfusion \
  --run-label "${LABEL}" \
...
echo "KAU_VECTOR_IMAGE_POSTFUSION_ALPHABET2_COMPLETE=${OUTPUT_ROOT}"
