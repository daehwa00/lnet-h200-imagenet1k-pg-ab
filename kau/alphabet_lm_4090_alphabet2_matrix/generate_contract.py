#!/usr/bin/env python3
"""Generate vector-addressed matrix-memory runtimes."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_alphabet2_matrix/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_alphabet2_matrix/campaign.runtime.json"
...
def _render() -> str:
    ...
    if (
        manifest.get("schema") != "lnet.kau.alphabet_lm.alphabet2_matrix.v1"
        or execution
        != [
            "alphabet2-matrix-k4v4-1m",
            "alphabet2-matrix-k4v4-2m",
            "alphabet2-matrix-k4v4-4m",
        ]
        ...
        or any(variant["slow_cnn_pole_matrix_key_width"] != 4 for variant in variants.values())
    ):
        raise RuntimeError("invalid matrix-memory campaign")
    ...
    for label in execution:
        ...
        runs[label] = {
            ...
            "display_name": f"RTX4090-S501-ALPHABET2-MatrixK4V4-{milestone}",
            "tags": ["RTX4090", "ALPHABET2", "MatrixK4V4", milestone, "seed501"],
        }
    runtime = {
        "schema": "lnet.kau.alphabet_lm.alphabet2_matrix.runtime.v1",
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale matrix-memory runtime", file=sys.stderr)
            return 1
        return 0
