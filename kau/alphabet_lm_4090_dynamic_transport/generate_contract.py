#!/usr/bin/env python3
"""Generate the frozen-Coordinate R16 dynamic-transport campaign."""
...
MANIFEST = ROOT / "kau/alphabet_lm_4090_dynamic_transport/campaign.json"
RUNTIME = ROOT / "kau/alphabet_lm_4090_dynamic_transport/campaign.runtime.json"
...
def _render() -> str:
    ...
    variant = variants["dynamic-transport-r16-4m"]
    if (
        execution != ["dynamic-transport-r16-4m"]
        or not variant["slow_cnn_pole_dynamic_transport"]
        or variant["slow_cnn_pole_transport_rank"] != 16
        or not variant["slow_cnn_pole_coordinate_read"]
        or not all(manifest["diagnostics"].values())
    ):
        raise RuntimeError("invalid dynamic-transport campaign")
    ...
    runtime = {
        ...
        "runs": {
            label: {
                ...
                "display_name": "RTX4090-S501-ALPHABET2-DynamicTransportR16-4M",
                "tags": ["RTX4090", "ALPHABET2", "DynamicTransport", "4M", "seed501"],
            }
        },
    }
...
def main() -> int:
    ...
    if args.check:
        if not RUNTIME.is_file() or RUNTIME.read_text(encoding="utf-8") != rendered:
            print("stale dynamic-transport runtime", file=sys.stderr)
            return 1
        return 0
