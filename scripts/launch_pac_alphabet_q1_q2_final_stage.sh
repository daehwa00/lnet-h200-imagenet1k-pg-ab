...
session="pac-alphabet-final-${stage}-${host_tag}-20260719"

worker_script="$project_root/scripts/run_pac_alphabet_q1_q2_final_worker.sh"
if [[ ! -x $worker_script ]]; then
  worker_script="$project_root/run_pac_alphabet_q1_q2_final_worker.sh"
fi
log_root="$project_root/.omx/logs/pac-alphabet-q1q2-final-20260719"
mkdir -p "$log_root"
