#!/usr/bin/env python3
"""Train H96 PG mode/path/post stages with a strict complex-linear D4 collapse."""
...
import run_a2d_deep4_calibrated_uniform_p96_phase_gated_h96_imagenet100 as base
import torch
...
VARIANT = "PGv2-H96-PathPG-CL-PostPG"
VARIANTS = (VARIANT,)
SEEDS = base.SEEDS
P = base.P
...
def _install_transition(stage: ComplexScanStage) -> None:
    baseline = stage.quadrant_path_mode_combiner
    if not isinstance(baseline, PhaseGatedModeResidualPathCollapse):
        message = "H96 path-PG transition requires the exact PGv2-H96 control"
    ...
    if type(post) is not FactorizedS2DPostFusionTransition:
        message = "H96 path-PG transition requires the factorized S2D control"
        raise TypeError(message)
...
def _assert_model(model: ComplexScanBackbone) -> None:
    base.control.control.stemres._assert_stem(model)
    for name in ("stage1", "stage2", "stage3"):
        stage = getattr(model, name)
        mixer = stage.quadrant_path_mode_combiner
        if not isinstance(mixer, PhaseGatedModePathResidualComplexLinearCollapse):
            message = f"{name} is missing its H96 PG/ComplexLinear transition"
            raise TypeError(message)
        if (
            mixer.mode.modes != P
            ...
            or mixer.collapse.input_modes != 4
            or mixer.collapse.output_modes != 1
            or not isinstance(stage.augmented, PhaseGatedS2DPostFusionTransition)
            or stage.augmented.post.hidden_modes != MODE_HIDDEN
        ):
            message = f"{name} changed the requested H96 transition contract"
            raise RuntimeError(message)
    if model.terminal.quadrant_path_mode_combiner is not None:
        message = "H96 PG/ComplexLinear experiment changed the terminal descriptor"
        raise RuntimeError(message)


def _build(variant: str, config: ComplexScanConfig) -> ComplexScanBackbone:
    if variant != VARIANT:
        message = f"unsupported H96 PG/ComplexLinear variant: {variant}"
        raise ValueError(message)
    model = base._build(base.VARIANT, config)
    for name in ("stage1", "stage2", "stage3"):
        _install_transition(getattr(model, name))
    _assert_model(model)
    return model
...
def _variant_config() -> dict[str, Any]:
    ...
    payload["backbone"]["name"] = "A2D-PGv2-H96-PathPG-CL-PostPG"
    ...
    transition["path_collapse"] = "shared-ComplexLinear-4-1"
    transition["path_collapse_initialization"] = "exact-mean: 0.25+0i"
    transition["postfusion"] = "PhaseGatedComplexFFNv2-96-96-96"
...
def _wandb_model_metrics(model: nn.Module) -> dict[str, float]:
    ...
    for index, name in enumerate(("stage1", "stage2", "stage3"), start=1):
        stage = getattr(model, name)
        mixer = stage.quadrant_path_mode_combiner
        if isinstance(mixer, PhaseGatedModePathResidualComplexLinearCollapse):
...
def _contract(args: Namespace) -> dict[str, Any]:
    ...
    config = base.control.control.stemres.uniform.base.PoleModelConfig(
        output_dim=100,
        stem_strides=(2, 2),
    )
    model = _build(VARIANT, config)
    payload["schema"] = "lnet.a2d.pgv2_h96.path_pg_complex_linear.imagenet100.v1"
    payload["evidence_status"] = "untrained H96 path-PG complex-linear candidate"
    ...
    payload["architecture"] = {
        VARIANT: (
            "Exact PGv2-H96-All3e-3 control with Stage 1-3 changed to H96 mode PG, "
            "shared residual path PG 4-8-4, shared strict ComplexLinear 4-to-1, "
            "unchanged S2D carry, and H96 phase-equivariant PG post-fusion. Terminal, "
            "head, optimizer, learning rate, and augmentation remain unchanged."
        )
    }
    digest = base.control.control.stemres.uniform.base.heads.harness._digest
    payload["source_sha256"]["pac_phase_gated_transition"] = digest(
        Path("src/lnet/pac_phase_gated_transition.py")
    )
    payload["source_sha256"]["h96_path_pg_complex_linear_runner"] = digest(Path(__file__))
    return payload


def main() -> None:
    base._configure_ramp()
    ramp = base.control.control.stemres.uniform.base
