...
def main():
    ...
    state_path=root/'observer-state-local-v2.json';state=read(state_path)
    cursor=state.get('cursor',read(root/'observer-state.json').get('cursor',0))
    ...
    while True:
        try:
            ...
            for event in view['events']:
                ...
                with (root/'events-local-v2.jsonl').open('a') as stream:stream.write(json.dumps(event)+'\n')
                job=current.get('job')
                ...
                if job in JOBS and job not in completed and current.get('stage') in ('waiting_wandb','training','waiting_metrics') and (job!=active_job or run is None):
                    ...
                    run=wandb.init(entity='daehwa',project=PROJECT,group=CAMPAIGN,id=identifier,resume='allow',
                        name=f'IN1K10-{model}-s{seed}-restart2',config={'model':model,'seed':int(seed),'scratch':True,
                ...
                if run and job==active_job:
                    ...
                    if result.get('completed_epochs')==100 and result.get('global_step')==50000 and result.get('status')=='completed':
                        run.summary.update({'completed_epochs':100,'global_step':50000,
                            'final/top1_percent':100*result['final_validation']['accuracy']})
                        run.finish();run=None
                        wb_api.flush()
                        saved=wb_api.run(f'daehwa/{PROJECT}/{run_id(job)}')
                        if saved.summary.get('completed_epochs')!=100 or saved.summary.get('global_step')!=50000:
            ...
            atomic(root/'observer-error-local-v2.json',{'time':time.time(),'error':type(error).__name__+': '+str(error)[:300]})
