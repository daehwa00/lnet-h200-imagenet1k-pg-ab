...
session="pac-full-early-q1-${stage}-${host_tag}-20260718"
tmux has-session -t "$session" 2>/dev/null && exit 0

worker_script="$project_root/scripts/run_full_early_q1_worker.sh"
if [[ ! -x $worker_script ]]; then
  worker_script="$project_root/run_full_early_q1_worker.sh"
fi
log_root="$project_root/.omx/logs/pac-full-early-q1-tuning-20260718"
mkdir -p "$log_root"
