#!/usr/bin/env python3
"""Forward/backward smoke test for A2D-ResAux1-LinearPath."""
...
import run_a2d_resaux1_linearpath_imagenet100 as candidate
import torch
...
def main() -> None:
    ...
    output = model(inputs)
    logits, loss, diagnostics = candidate.base.heads._training_objective(
        model,
        output,
        targets,
        targets.flip(0),
    ...
    if not gradients or not all(
        ...
        message = "A2D LinearPath produced invalid gradients"
        raise RuntimeError(message)
    if not bool(torch.isfinite(logits).all() and torch.isfinite(loss)):
        message = "A2D LinearPath produced non-finite outputs"
        raise RuntimeError(message)
    print(
        candidate.VARIANT,
        "parameters=",
