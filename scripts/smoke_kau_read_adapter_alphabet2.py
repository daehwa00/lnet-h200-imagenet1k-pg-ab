#!/usr/bin/env python3
"""CUDA train/save/load smoke for the pole-wise read-adapter ALPHABET-2."""
...
from lnet.alphabet_lm import LaplaceMambaLMConfig, ReadAdaptedImagePostFusionAlphabet2LM
...
def main() -> None:
    ...
    model = ReadAdaptedImagePostFusionAlphabet2LM(config).cuda().train()
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        restored = ReadAdaptedImagePostFusionAlphabet2LM(config).cuda().eval()
    ...
    if not torch.isfinite(restored_logits).all():
        raise RuntimeError("restored read-adapter ALPHABET-2 output is non-finite")
    details = f"loss={float(loss.detach()):.6f},shape={tuple(logits.shape)}"
    print(f"READ_ADAPTER_ALPHABET2_SMOKE={details}", flush=True)
