"""Fast UCR-16 screen for the two-layer-head state-only pooling V9.6."""
...
from optimization.state_only_pole_pooling_v96 import StateOnlyPolePoolingV96ALPHABET
from scripts import pac_logsig_v6_ucr16_fast as harness
from scripts import pac_pole_attention_ucr16_fast as base

ROOT = Path(".omx/results/pac-logsig-v96-ucr16-fast-20260721")
VARIANT = "v96_state_only_two_layer_head"
...
def _source_hashes() -> dict[str, str]:
    ...
    paths = (
        ...
        Path("scripts/pac_logsig_v96_ucr16_fast.py"),
    )
    return {
...
def _design() -> dict[str, object]:
    candidate = StateOnlyPolePoolingV96ALPHABET(
        1,
        base.MODEL_DIM,
        base.MODES,
        5,
    ...
    return {
        "schema": "pac_logsig_v96_ucr16_fast_contract.v1",
        "purpose": "validation-only V9.6 screen; no automatic Q1 promotion",
        ...
        "controlled_difference_from_v95": (
            "remove the softmax-invariant final scorer bias and replace the single linear "
            "prediction head with Linear(6M,128)-SiLU-Linear(128,K)"
        ),
        "model_contract": {
            "edge_lift": (
...
def jobs() -> list[base.Job]:
    ...
    return [
        base.Job(
            key=f"logsig_v96_ucr16_fast:{dataset}:{VARIANT}:seed{seed}",
            dataset=dataset,
...
def _build_model(job: base.Job, input_dim: int, output_dim: int):  # noqa: ANN202
    if job.variant != VARIANT:
        message = f"unknown V9.6 fast-screen variant: {job.variant}"
        raise ValueError(message)
    return StateOnlyPolePoolingV96ALPHABET(
        input_dim,
        job.model_dim,
        job.modes,
        output_dim,
...
def report(root: Path) -> dict[str, object]:
    ...
    if campaign_status["done"] is not True:
        message = f"refusing to report incomplete V9.6 results: {campaign_status}"
    ...
    if len(rows) != len(expected) or {str(row.get("job_key")) for row in rows} != expected:
        message = "V9.6 completed rows do not match the sealed job set"
        raise RuntimeError(message)
    digest = design_sha256()
    for row in rows:
        if (
            ...
            message = f"invalid, stale, or contaminated V9.6 row: {row.get('job_key')}"
            raise RuntimeError(message)
...
def main() -> None:
    ...
    harness.REPORT_SCHEMA = "pac_logsig_v96_ucr16_fast_report.v1"
    harness.DELTA_FIELD = "v96_minus_h_compact"
