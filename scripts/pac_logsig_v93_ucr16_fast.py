"""Fast UCR-16 screen for V9.3 physical-time attentive pole pooling."""
...
from optimization.attentive_pole_pooling import AttentivePolePoolingALPHABET
from scripts import pac_logsig_v6_ucr16_fast as harness
from scripts import pac_pole_attention_ucr16_fast as base

ROOT = Path(".omx/results/pac-logsig-v93-ucr16-fast-20260721")
VARIANT = "v93_attentive_pole_pooling"
...
def _source_hashes() -> dict[str, str]:
    ...
    paths = (
        ...
        Path("scripts/pac_logsig_v93_ucr16_fast.py"),
    )
    return {
...
def _design() -> dict[str, object]:
    candidate = AttentivePolePoolingALPHABET(
        1,
        base.MODEL_DIM,
        base.MODES,
        5,
    ...
    return {
        "schema": "pac_logsig_v93_ucr16_fast_contract.v1",
        "purpose": "validation-only V9.3 screen; no automatic Q1 promotion",
        ...
        "controlled_difference_from_v9": (
            "remove both pair-pole CNN blocks and the degree-8 Legendre WLS; replace them "
            "with physical-time mean, learned relevance pooling, terminal state, attended "
            "time, and duration"
        ),
        "model_contract": {
            "edge_lift": "unchanged V9 level/velocity lift and residual causal DWConv1d(k=5)",
            "normalization": "unchanged parameter-free amplitude-preserving bounded map",
            "modal_analysis": "unchanged invertible square orthogonal analysis with D=2M",
...
def jobs() -> list[base.Job]:
    ...
    return [
        base.Job(
            key=f"logsig_v93_ucr16_fast:{dataset}:{VARIANT}:seed{seed}",
            dataset=dataset,
...
def _build_model(job: base.Job, input_dim: int, output_dim: int):  # noqa: ANN202
    if job.variant != VARIANT:
        message = f"unknown V9.3 fast-screen variant: {job.variant}"
        raise ValueError(message)
    return AttentivePolePoolingALPHABET(
        input_dim,
        job.model_dim,
        job.modes,
        output_dim,
...
def report(root: Path) -> dict[str, object]:
    ...
    if campaign_status["done"] is not True:
        message = f"refusing to report incomplete V9.3 results: {campaign_status}"
    ...
    if len(rows) != len(expected) or {str(row.get("job_key")) for row in rows} != expected:
        message = "V9.3 completed rows do not match the sealed job set"
        raise RuntimeError(message)
    digest = design_sha256()
    for row in rows:
        if (
            ...
            message = f"invalid, stale, or contaminated V9.3 row: {row.get('job_key')}"
            raise RuntimeError(message)
...
def main() -> None:
    ...
    harness.REPORT_SCHEMA = "pac_logsig_v93_ucr16_fast_report.v1"
    harness.DELTA_FIELD = "v93_minus_h_compact"
