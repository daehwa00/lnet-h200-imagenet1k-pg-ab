...
"""Run the coherence-gated Complex Interaction Block CIFAR-100 screen."""
...
def _config() -> ComplexScanConfig:
    return ComplexScanConfig(interaction_ranks=(8, 8))


def _build(variant: str, config: ComplexScanConfig) -> ComplexScanBackbone:
    if variant != VARIANTS[0]:
        message = f"unknown complex interaction variant: {variant}"
...
def _build_optimizer(
    ...
    for name, parameter in model.named_parameters():
        if "damping_logits" in name or "phase_" in name:
            ...
            "analysis" in name
            or "interaction.input_projection" in name
            or "interaction.output_projection" in name
        ):
...
def _contract(args: Namespace) -> dict[str, Any]:
    ...
    payload = {
        "schema": "lnet.complex_scan.interaction.cifar100.v1",
        "evidence_status": "validation-only 50-epoch complex composition screen",
        ...
        "architecture": {
            ...
            "coherence_ranks": [8, 8],
            "coherence": "a*conj(b)",
            "context": "log1p(|U|^2), Re(a*conj(b)), Im(a*conj(b))",
            "gate": "2*sigmoid(W*context+b), identity initialized",
        ...
        "references": {
            ...
            "complex_amp_phase_wide_seed401_50epoch": 0.4878,
        },
        "recipe": {
            "epochs": args.epochs,
