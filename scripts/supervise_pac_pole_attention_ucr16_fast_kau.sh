...
project_root=${1:-.}
root=.omx/results/pac-pole-attention-ucr16-fast-20260720
...
workers=6
log=.omx/logs/pac-pole-attention-ucr16-fast-kau-20260720.log
...
for index in $(seq 0 $((workers - 1))); do
  ...
  session=$(printf 'pac-pole-attention-ucr16-fast-%02d-20260720' "$index")
...
PY
  ...
  tmux new-session -d -s "$session" \
    "cd '$repo' && for retry in 1 2 3; do OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 PYTHONPATH=src:. CUDA_VISIBLE_DEVICES=0 '$python_bin' scripts/pac_pole_attention_ucr16_fast.py worker --root '$root' --manifest '$manifest' --device cuda >>'$logfile' 2>&1; done"
done
REMOTE
}
...
while true; do
  sync_results
  status=$(PYTHONPATH=src:. "$local_python" scripts/pac_pole_attention_ucr16_fast.py \
    status --root "$root")
...
PYTHONPATH=src:. "$local_python" scripts/pac_pole_attention_ucr16_fast.py \
  report --root "$root" | tee -a "$log"
printf '%s pole-attention UCR-16 screen complete\n' "$(date --iso-8601=seconds)" | tee -a "$log"
