...
project_root=${1:-.}
root=.omx/results/pac-h-compact-lag124-q1-final-20260721
baseline_root=.omx/results/pac-baseline-fairness-maximal-20260714
module=lnet.pac_h_compact_lag124_q1_cli
campaign=lnet.pac_h_compact_lag124_q1_campaign
lanes=14
poll_seconds=${PAC_H_LAG124_Q1_POLL_SECONDS:-30}
...
log=.omx/logs/pac-h-compact-lag124-q1-final-20260721-supervisor.log
...
sync_sources() {
  local files=(
    ...
    src/lnet/pac_compiled_lag124_moments.py
    src/lnet/pac_h_compact_lag124.py
    src/lnet/pac_h_compact_lag124_q1_campaign.py
    src/lnet/pac_h_compact_lag124_q1_cli.py
    src/lnet/pac_tight_frame_models.py
    src/lnet/pac_two_tap_q1_campaign.py
  )
  local file
...
ensure_local_workers() {
  ...
  for index in $(seq 0 7); do
    ...
    session=$(printf 'pac-h-lag124-%s-local-%02d' "$stage" "$index")
    ...
    tmux new-session -d -s "$session" \
      "cd '$project_root' && OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 PYTHONSAFEPATH=1 PYTHONPATH=src CUDA_VISIBLE_DEVICES=0 '$local_python' -m '$module' --stage worker --output-root '$root' --manifest '$manifest' --device cuda >>'$logfile' 2>&1"
  done
}
...
ensure_remote_worker() {
  ...
  manifest=$(printf '%s/%s/manifests/worker-%02d.jsonl' "$root" "$stage" "$index")
  session=$(printf 'pac-h-lag124-%s-%s-%02d' "$stage" "$label" "$index")
  logfile=$(printf '%s/%s/logs/%s-%02d.log' "$root" "$stage" "$label" "$index")
  ssh -i "$key" -p "$port" -o BatchMode=yes -o ConnectTimeout=10 "$host" bash -s -- \
    "$repo" "$python_path" "$manifest" "$stage" "$session" "$logfile" "$gpu" "$root" "$module" <<'REMOTE'
set -euo pipefail
repo=$1; python_path=$2; manifest=$3; stage=$4; session=$5; logfile=$6; gpu=$7; root=$8; module=$9
cd "$repo"
[[ -f $manifest ]] || exit 0
...
mkdir -p "$(dirname "$logfile")"
tmux new-session -d -s "$session" \
  "cd '$repo' && OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 PYTHONSAFEPATH=1 PYTHONPATH=src CUDA_VISIBLE_DEVICES='$gpu' '$python_path' -m '$module' --stage worker --output-root '$root' --manifest '$manifest' --device cuda >>'$logfile' 2>&1"
REMOTE
}

stage_done() {
  local stage=$1
  PYTHONPATH=src "$local_python" - "$root" "$stage" "$campaign" <<'PY'
import importlib
import sys
from pathlib import Path
...
run_stage() {
  ...
  while true; do
    collect_stage "$stage"
    snapshot=$(PYTHONPATH=src "$local_python" -m "$module" --stage status --output-root "$root")
    printf '%s stage=%s %s\n' "$(date --iso-8601=seconds)" "$stage" "$(tr '\n' ' ' <<<"$snapshot")" | tee -a "$log"
    stage_done "$stage" && break
    ensure_local_workers "$stage"
    if [[ -f $root/$stage/manifests/worker-13.jsonl ]]; then
      # RTX 4090 policy: two independent workers per physical GPU.  This fills
      # host-side/data-pipeline bubbles without heavy oversubscription.
      ...
      # Preserve the already sealed 11-lane Stage-1 assignment.
      ensure_remote_worker "$local_gpu_host" 5003 "$local_gpu_key" "$local_gpu_repo" "$local_gpu_python" "$stage" 8 0 local_gpu || true
      ensure_remote_worker "$local_gpu_host" 5003 "$local_gpu_key" "$local_gpu_repo" "$local_gpu_python" "$stage" 9 1 local_gpu || true
      ensure_remote_worker "$kau_host" 8589 "$kau_key" "$kau_repo" "$kau_python" "$stage" 10 0 kau || true
    fi
    sleep "$poll_seconds"
  done
}
...
if [[ ! -f $root/stage1/contract.json ]]; then
  PYTHONPATH=src "$local_python" -m "$module" --stage enqueue-stage1 --output-root "$root" --lanes "$lanes" | tee -a "$log"
fi
run_stage stage1
PYTHONPATH=src "$local_python" -m "$module" --stage select-stage1 --output-root "$root" --lanes "$lanes" | tee -a "$log"
run_stage stage2
PYTHONPATH=src "$local_python" -m "$module" --stage select-stage2 --output-root "$root" --baseline-root "$baseline_root" | tee -a "$log"
PYTHONPATH=src "$local_python" -m "$module" --stage enqueue-final --output-root "$root" --baseline-root "$baseline_root" --lanes "$lanes" | tee -a "$log"
mkdir -p "$root/provenance"
rsync -a "$baseline_root/provenance/" "$root/provenance/"
run_stage final
printf '%s H-compact lag-(1,2,4) Q1 execution complete\n' "$(date --iso-8601=seconds)" | tee -a "$log"
