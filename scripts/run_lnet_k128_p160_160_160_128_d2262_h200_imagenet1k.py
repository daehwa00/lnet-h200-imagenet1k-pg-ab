#!/usr/bin/env python3
"""Train the K128/P160-160-160-128/D2262 H200 confirmation seeds."""
...
import run_a2d_r2k3_d2262_p_schedule_imagenet100 as anchor
...
class PrimaryLogitsAdapter(nn.Module):
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        if not isinstance(output, tuple) or len(output) != 5:
            raise RuntimeError("K128 ImageNet-1K model lost its five-output Q-head contract")
        logits = output[0]
        if not isinstance(logits, Tensor):
            raise TypeError("K128 ImageNet-1K primary logits are not a tensor")
        return logits
...
def _model_spec(key: str) -> SimpleNamespace:
    if key != MODEL_KEY:
        raise ValueError(f"unsupported K128 H200 model: {key}")
    return SimpleNamespace(backend="internal", display_name=DISPLAY_NAME, precision="bfloat16")
...
def _build_model(
    ...
    if key != MODEL_KEY or num_classes != 1000:
        raise ValueError("K128 H200 builder received an incompatible task")
    model = anchor._build(VARIANT, runtime.model_config(output_dim=num_classes))
    parameters = sum(parameter.numel() for parameter in model.parameters())
    if parameters != EXPECTED_PARAMETERS:
        raise RuntimeError(
            f"K128 ImageNet-1K parameter count changed: {parameters} != {EXPECTED_PARAMETERS}"
        )
    return PrimaryLogitsAdapter(model.prepare_for_compiled_training_())
    ...
    os.environ.update(
        {
            ...
            "H200_BASELINE_DISPLAY_NAME": f"H200-LNet-I1K-K128-s{seed}",
...
def main() -> None:
    ...
    print("LNET_K128_H200_RESULT_JSON=" + json.dumps(result, sort_keys=True), flush=True)
