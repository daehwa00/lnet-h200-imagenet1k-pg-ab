#!/usr/bin/env python3
"""CUDA train/save/load smoke for pole-axis whitened J2 ALPHABET-2."""
...
from lnet.alphabet_lm import LaplaceMambaLMConfig, TemporallyWhitenedImagePostFusionAlphabet2LM
...
def main() -> None:
    ...
    model = TemporallyWhitenedImagePostFusionAlphabet2LM(config).cuda().train()
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        restored = TemporallyWhitenedImagePostFusionAlphabet2LM(config).cuda().eval()
    ...
    if not torch.isfinite(restored_logits).all():
        raise RuntimeError("restored temporal-whitening ALPHABET-2 output is non-finite")
    details = f"loss={float(loss.detach()):.6f},shape={tuple(logits.shape)}"
    print(f"TEMPORAL_WHITENING_ALPHABET2_SMOKE={details}", flush=True)
