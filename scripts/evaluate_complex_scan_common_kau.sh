...
goal_root="${GOAL_ROOT:-$script_root/.omx/goals/performance/complex-scan-common-kau}"
baseline_root="${BASELINE_ROOT:-$goal_root/frozen_runtime}"
candidate_root="${CANDIDATE_ROOT:-$goal_root/candidate_runtime}"
artifact_root="${ARTIFACT_ROOT:-$goal_root/artifacts}"
harness="$script_root/scripts/benchmark_complex_scan_common_kau.py"

default_python=/home/daehwa/anaconda3/envs/alphabet/bin/python
...
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-${KAU_GPU:-0}}"
...
TORCHINDUCTOR_CACHE_DIR="$artifact_root/inductor-candidate" \
  "$python_bin" "$harness" benchmark \
    --role candidate \
    --runtime-root "$candidate_root" \
    --fixture "$fixture" \
    --reference-evidence "$baseline_evidence" \
...
PYTHONPATH="$candidate_root/src" "$python_bin" "$harness" evaluate \
  ...
  --minimum-speedup 1.15 \
  --maximum-memory-ratio 1.0 \
  --maximum-logits-error 2e-2 \
  --maximum-loss-error 5e-3 \
