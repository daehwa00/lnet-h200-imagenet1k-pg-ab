#!/usr/bin/env python3
"""Forward/backward smoke test for A2D-ResAux1."""
...
import run_a2d_resaux1_imagenet100 as candidate
import torch
...
def main() -> None:
    ...
    if not finite_gradients or not all(
        bool(torch.isfinite(gradient).all()) for gradient in finite_gradients
    ):
        message = "A2D-ResAux1 produced invalid gradients"
        raise RuntimeError(message)
    if not bool(torch.isfinite(logits).all() and torch.isfinite(loss)):
        message = "A2D-ResAux1 produced non-finite outputs"
        raise RuntimeError(message)
    print(
        candidate.VARIANT,
        "parameters=",
