"""Fast UCR-16 screen for the physical-time single-writer LogSignature V7."""
...
from optimization.physical_single_writer_log_signature import (
    PhysicalSingleWriterLogSignatureALPHABET,
)
from scripts import pac_logsig_v6_ucr16_fast as harness
from scripts import pac_pole_attention_ucr16_fast as base

ROOT = Path(".omx/results/pac-logsig-v7-ucr16-fast-20260721")
VARIANT = "v7_physical_single_writer"
...
def _source_hashes() -> dict[str, str]:
    ...
    paths = (
        ...
        Path("scripts/pac_logsig_v7_ucr16_fast.py"),
    )
    return {
...
def _design() -> dict[str, object]:
    candidate = PhysicalSingleWriterLogSignatureALPHABET(
        1,
        base.MODEL_DIM,
        base.MODES,
        5,
    ...
    return {
        "schema": "pac_logsig_v7_ucr16_fast_contract.v1",
        "purpose": "validation-only V7 screen; no automatic Q1 promotion",
        ...
        "model_contract": {
            "edge_lift": "learned level plus physical velocity using the actual interval",
            ...
            "modal_analysis": "square semi-orthogonal D=2M frame",
            "pole_initialization": "log-spaced damping and frequency-spread learned poles",
            "modal_core": "one exact-ZOH writer scan",
            "content_readout": "physical-time weighted mean plus terminal synthesized state",
            "dynamics_readout": "observation-duration-weighted pole-motion LogSignature",
            "head": "single linear map from 2D+5M",
            "reader": False,
            "second_scan": False,
            "fixed_lags": False,
        },
...
def jobs() -> list[base.Job]:
    ...
    return [
        base.Job(
            key=f"logsig_v7_ucr16_fast:{dataset}:{VARIANT}:seed{seed}",
            dataset=dataset,
            ...
            estimated_seconds=UCR_SECONDS[dataset] * 0.65,
...
def _build_model(job: base.Job, input_dim: int, output_dim: int):  # noqa: ANN202
    if job.variant != VARIANT:
        message = f"unknown V7 fast-screen variant: {job.variant}"
        raise ValueError(message)
    return PhysicalSingleWriterLogSignatureALPHABET(
        input_dim,
        job.model_dim,
        job.modes,
        output_dim,
...
def main() -> None:
    ...
    harness.REPORT_SCHEMA = "pac_logsig_v7_ucr16_fast_report.v1"
    harness.DELTA_FIELD = "v7_minus_h_compact"
