#!/usr/bin/env python3
"""CUDA train/save/load smoke for the image-PostFusion ALPHABET-2 block."""
...
from lnet.alphabet_lm import ImagePostFusionAlphabet2LM, LaplaceMambaLMConfig
...
def main() -> None:
    ...
    model = ImagePostFusionAlphabet2LM(config).cuda().train()
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        restored = ImagePostFusionAlphabet2LM(config).cuda().eval()
    ...
    if not torch.isfinite(restored_logits).all():
        raise RuntimeError("restored image-PostFusion ALPHABET-2 output is non-finite")
    details = f"loss={float(loss.detach()):.6f},shape={tuple(logits.shape)}"
    print(f"IMAGE_POSTFUSION_ALPHABET2_SMOKE={details}", flush=True)
