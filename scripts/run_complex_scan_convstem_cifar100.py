...
"""Run the conv-only stem Complex-Linear CIFAR-100 validation screen."""
...
def _config() -> ComplexScanConfig:
    return ComplexScanConfig(stem="conv_only")


def _build(variant: str, config: ComplexScanConfig) -> ComplexScanBackbone:
    if variant != VARIANTS[0]:
        message = f"unknown conv-only stem variant: {variant}"
...
def _contract(args: Namespace) -> dict[str, Any]:
    ...
    payload = {
        "schema": "lnet.complex_scan.conv_only_stem.cifar100.v1",
        ...
        "ablation": {
            ...
            "changed": "stem only",
            "removed": ["LayerNorm2d after conv1", "GELU", "LayerNorm2d after conv2"],
