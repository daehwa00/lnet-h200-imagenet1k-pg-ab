#!/usr/bin/env python3
"""CUDA train/save/load smoke for exact pole-wise dynamic-delta ALPHABET-2."""
...
from lnet.alphabet_lm import DynamicDeltaImagePostFusionAlphabet2LM, LaplaceMambaLMConfig
...
def main() -> None:
    ...
    config = LaplaceMambaLMConfig(
        ...
        state_size=2,
        head_width=8,
        dynamic_delta_hidden=16,
        conv_width=3,
        context_length=64,
        minimum_half_life=4.0,
        maximum_half_life=64.0,
    )
    model = DynamicDeltaImagePostFusionAlphabet2LM(config).cuda().train()
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        restored = DynamicDeltaImagePostFusionAlphabet2LM(config).cuda().eval()
    ...
    if not torch.isfinite(restored_logits).all():
        raise RuntimeError("restored dynamic-delta ALPHABET-2 output is non-finite")
    details = f"loss={float(loss.detach()):.6f},shape={tuple(logits.shape)}"
    print(f"DYNAMIC_DELTA_ALPHABET2_SMOKE={details}", flush=True)
