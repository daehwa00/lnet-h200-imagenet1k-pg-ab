#!/usr/bin/env python3
"""One real-ImageNet BF16 compiled step for a multi-pole excitation arm."""
...
import run_a2d_deep4_calibrated_uniform_p96_h96_multi_pole_excitation_imagenet100 as runner
import torch
...
def _mixed_loss(output: object, targets: Tensor) -> Tensor:
    if not isinstance(output, tuple) or len(output) != 5:
        message = "multi-pole smoke requires the established five-output classifier"
...
def main() -> None:
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        message = "multi-pole smoke requires exactly one visible CUDA device"
        raise RuntimeError(message)
    os.environ["LNET_COMPILE_MODE"] = args.compile_mode
    torch.manual_seed(821)
    torch.cuda.manual_seed_all(821)
    ...
    multi_stages = [
        module for module in model.modules() if isinstance(module, MultiPoleExcitationStage)
    ]
    pg_blocks = [module for module in model.modules() if isinstance(module, PhaseGatedComplexFFN)]
    if (
        missing
        or len(multi_stages) != 4
        or any(stage.diagnostic_updates.item() < 1 for stage in multi_stages)
        or not pg_blocks
        or any(block.diagnostic_updates.item() < 1 for block in pg_blocks)
    ):
        message = f"multi-pole smoke has inactive parameters or blocks: {missing[:8]}"
        raise RuntimeError(message)

    checkpoint = args.root / "multi-pole-smoke.pt"
    ...
    payload = {
        ...
        "multiplicity": runner.MULTIPLICITY[args.variant],
        "batch_size": args.batch_size,
        "compile_mode": args.compile_mode,
        "device": torch.cuda.get_device_name(),
        "loss": float(loss.detach()),
        "phase_gated_blocks": len(pg_blocks),
        "multi_pole_stages": len(multi_stages),
