#!/usr/bin/env python3
"""Train the exact PGv2-H96-K3-RMSMatch backbone with a Q4-only affine head."""
...
VARIANT = "PGv2-H96-K3-RMSMatch-Q4Affine"
VARIANTS = (VARIANT,)
...
class StageOnlyAffineClassifier(head_runner.A2DAffineQClassifier):
    """Select one ordered stage descriptor and apply a standardized affine head."""

    def __init__(self, output_dim: int, *, stage_index: int) -> None:
        if not 0 <= stage_index < STAGE_COUNT:
            message = f"stage index must be in [0, {STAGE_COUNT})"
    ...
    def select_stage(self, descriptor: Tensor) -> Tensor:
        if descriptor.shape[-1] != FULL_DESCRIPTOR_DIM:
            message = (
                "stage-only affine head requires the ordered Q1-Q4 descriptor "
                f"with width {FULL_DESCRIPTOR_DIM}"
            )
            raise ValueError(message)
        start = self.stage_index * Q4_DIM
        return descriptor[..., start : start + Q4_DIM]

    def forward(self, descriptor: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        return super().forward(self.select_stage(descriptor))


class Q4OnlyAffineClassifier(StageOnlyAffineClassifier):
    """Select terminal Q4 and apply one standardized affine classifier."""

    def __init__(self, output_dim: int) -> None:
        super().__init__(output_dim, stage_index=3)
...
def _contract(args: Namespace) -> dict[str, Any]:
    ...
    payload["schema"] = "lnet.a2d.pgv2_h96.k3_rmsmatch_q4_affine.imagenet100.v1"
    payload["evidence_status"] = "untrained terminal-Q4 affine-head ablation"
