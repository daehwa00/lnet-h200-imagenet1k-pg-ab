#!/usr/bin/env python3
"""Run one D2262 P-schedule model with H200 control and relay identities."""
...
def _runtime() -> dict[str, Any]:
    value = os.environ.get("H200_D2262_P_SCHEDULE_WANDB_RUNTIME")
    if not value:
...
def _metadata(variant: str, seed: int) -> dict[str, Any]:
    ...
    if variant not in payload["training"]["variants"] or seed != payload["training"]["seed"]:
        raise RuntimeError(f"unregistered H200 D2262 P-schedule run: {variant}/seed{seed}")
    ...
    if any(os.environ.get(name) != value for name, value in expected.items()):
        raise RuntimeError("H200 D2262 P-schedule W&B environment changed")
...
def _initialize_required_wandb_run(
    ...
    *,
    ...
    try:
        ...
        print(f"H200_D2262_P_WANDB_DEGRADED={type(error).__name__}", flush=True)
        return None
    if run is None or not run.url:
        print("H200_D2262_P_WANDB_DEGRADED=missing_run_url", flush=True)
        return None
...
def _write_heartbeat() -> None:
    _atomic_json(
        ...
        {
            "schema": "lnet.h200.imagenet100.d2262_p_schedule.heartbeat.v1",
            "campaign_id": _runtime()["campaign_id"],
            "target_commit": os.environ["H200_EXPECTED_COMMIT"],
...
def _selected_variant() -> str:
    try:
        ...
        raise RuntimeError("H200 D2262 P-schedule worker requires --variants") from error
    values: list[str] = []
    ...
    if len(values) != 1 or values[0] not in experiment.VARIANTS:
        raise RuntimeError("H200 worker requires exactly one registered P-schedule variant")
    return values[0]
...
def main() -> None:
    ...
    if initial_stop is not None:
        ...
        return
    print(f"H200_D2262_P_MODEL_START={selected}", flush=True)
    try:
        experiment.main()
