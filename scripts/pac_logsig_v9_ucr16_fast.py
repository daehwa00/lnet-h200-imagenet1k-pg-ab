"""Fast UCR-16 screen for the full-pole-trajectory Legendre V9."""
...
from optimization.legendre_pole_trajectory import LegendrePoleTrajectoryALPHABET
from scripts import pac_logsig_v6_ucr16_fast as harness
from scripts import pac_pole_attention_ucr16_fast as base

ROOT = Path(".omx/results/pac-logsig-v9-ucr16-fast-20260721")
VARIANT = "v9_legendre_pole_trajectory"
...
def _source_hashes() -> dict[str, str]:
    ...
    paths = (
        ...
        Path("scripts/pac_logsig_v9_ucr16_fast.py"),
    )
    return {
...
def _design() -> dict[str, object]:
    candidate = LegendrePoleTrajectoryALPHABET(
        1,
        base.MODEL_DIM,
        base.MODES,
        5,
    ...
    return {
        "schema": "pac_logsig_v9_ucr16_fast_contract.v1",
        "purpose": "validation-only V9 screen; no automatic Q1 promotion",
        ...
        "model_contract": {
            "edge_lift": "level plus physical-time velocity and residual causal DWConv1d(k=5)",
            ...
            "pole_trajectory": "all interleaved real/imaginary states after fixed-scale asinh",
            "local_readout": (
                "two residual pair-grouped causal Conv1d(k=3) blocks with pointwise mixing "
                "and linear time-metadata gates"
            ),
            "temporal_readout": (
                "degree-8 shifted-Legendre physical-time WLS using Cholesky solve and ridge 1e-4"
            ),
            "terminal_readout": "last-valid compressed pole state",
            "time_readout": "log total duration and observed-duration ratio",
            "head": "single linear map from 546 features",
            "state_scale": 1.0,
            "reference_step": 1.0,
            "synthesis": False,
            "reader": False,
...
def jobs() -> list[base.Job]:
    ...
    return [
        base.Job(
            key=f"logsig_v9_ucr16_fast:{dataset}:{VARIANT}:seed{seed}",
            dataset=dataset,
...
def _build_model(job: base.Job, input_dim: int, output_dim: int):  # noqa: ANN202
    if job.variant != VARIANT:
        message = f"unknown V9 fast-screen variant: {job.variant}"
        raise ValueError(message)
    return LegendrePoleTrajectoryALPHABET(
        input_dim,
        job.model_dim,
        job.modes,
        output_dim,
...
def report(root: Path) -> dict[str, object]:
    ...
    if campaign_status["done"] is not True:
        message = f"refusing to report incomplete V9 results: {campaign_status}"
    ...
    if len(rows) != len(expected) or {str(row.get("job_key")) for row in rows} != expected:
        message = "V9 completed rows do not match the sealed job set"
        raise RuntimeError(message)
    digest = design_sha256()
    for row in rows:
        if (
            ...
            message = f"invalid, stale, or contaminated V9 row: {row.get('job_key')}"
            raise RuntimeError(message)
...
def main() -> None:
    ...
    harness.REPORT_SCHEMA = "pac_logsig_v9_ucr16_fast_report.v1"
    harness.DELTA_FIELD = "v9_minus_h_compact"
