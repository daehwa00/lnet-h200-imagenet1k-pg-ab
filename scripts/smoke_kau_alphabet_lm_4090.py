    ...
    if args.only in {
        ...
        cascade = AlphabetLM(
            AlphabetLMConfig(
                ...
                additional_slow_cnn_pole_use_recurrence=args.only
                == "cnn_pole_p128_6bank_cascade_p128",
            )
        )
        full_microbatch = torch.randint(32_768, (8, 2_049), device="cuda")
        results[args.only] = _step(cascade, full_microbatch)
    if args.only in {
        "slow_r2k4_p128",
        "slow_r2k4_p128_no_recurrence",
        "slow_r4k4_p128",
        ...
        factorized = AlphabetLM(
            AlphabetLMConfig(
                ...
                slow_cnn_pole_upper_blocks=4,
                slow_cnn_pole_beta_initial=0.01,
                slow_cnn_pole_use_recurrence="no_recurrence" not in args.only,
                slow_cnn_pole_minimum_half_life=1.0,
                slow_cnn_pole_maximum_half_life=256.0,
                slow_cnn_pole_reader="factorized_complex",
                slow_cnn_pole_reader_rank=2 if "r2k4" in args.only else 4,
            )
        )
        full_microbatch = torch.randint(32_768, (8, 2_049), device="cuda")
        results[args.only] = _step(factorized, full_microbatch)
    if args.only == "all":
        torch.manual_seed(501)
