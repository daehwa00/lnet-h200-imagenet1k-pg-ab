...
def main() -> None:
    ...
    if args.only in {"cnn_pole_p128_6bank", "cnn_pole_p128_6bank_no_recurrence"}:
        torch.manual_seed(501)
        cnn_pole = AlphabetLM(
            AlphabetLMConfig(
                reader_type="dense_k3",
                ...
                cnn_pole_use_recurrence=args.only == "cnn_pole_p128_6bank",
                cnn_pole_minimum_half_life=8.0,
                cnn_pole_maximum_half_life=4_096.0,
            )
        )
        full_microbatch = torch.randint(32_768, (8, 2_049), device="cuda")
        results[args.only] = _step(cnn_pole, full_microbatch)
    if args.only in {
        "cnn_pole_p128_6bank_slow_p128",
        "cnn_pole_p128_6bank_slow_p128_no_recurrence",
    }:
        ...
        combined = AlphabetLM(
            AlphabetLMConfig(
                ...
                slow_cnn_pole_use_recurrence=args.only
                == "cnn_pole_p128_6bank_slow_p128",
        ...
        full_microbatch = torch.randint(32_768, (8, 2_049), device="cuda")
        results[args.only] = _step(combined, full_microbatch)
    if args.only in {"alphabet2_anchor_q", "alphabet2_token_q"}:
        torch.manual_seed(501)
        addressed = AlphabetLM(
            AlphabetLMConfig(
                reader_type="dense_k3",
                memory_layout="local_only",
                cnn_pole_memory=True,
                ...
                slow_cnn_pole_maximum_half_life=256.0,
                slow_cnn_pole_query=(
                    "anchor" if args.only == "alphabet2_anchor_q" else "token"
                ),
                slow_cnn_pole_query_rho=0.5,
            )
        )
        full_microbatch = torch.randint(32_768, (8, 2_049), device="cuda")
        results[args.only] = _step(addressed, full_microbatch)
    if args.only == "alphabet2_qk":
        torch.manual_seed(501)
        qk = AlphabetLM(
            AlphabetLMConfig(
                reader_type="dense_k3",
        ...
        full_microbatch = torch.randint(32_768, (8, 2_049), device="cuda")
        results[args.only] = _step(qk, full_microbatch)
    if args.only == "alphabet2_vector_d4":
        torch.manual_seed(501)
        vector = AlphabetLM(
            AlphabetLMConfig(
                reader_type="dense_k3",
        ...
        full_microbatch = torch.randint(32_768, (8, 2_049), device="cuda")
        results[args.only] = _step(vector, full_microbatch)
    if args.only == "alphabet2_matrix_k4v4":
        torch.manual_seed(501)
        matrix = AlphabetLM(
            AlphabetLMConfig(
                reader_type="dense_k3",
    ...
    if args.only == "alphabet2_vector_pole_r4":
        ...
        full_microbatch = torch.randint(32_768, (8, 2_049), device="cuda")
        results[args.only] = _step(vector_pole, full_microbatch)
    if args.only == "alphabet2_complex_vector_r4":
        torch.manual_seed(501)
        complex_vector = AlphabetLM(
            AlphabetLMConfig(
                reader_type="dense_k3",
                memory_layout="local_only",
                cnn_pole_memory=True,
        ...
        full_microbatch = torch.randint(32_768, (8, 2_049), device="cuda")
        results[args.only] = _step(complex_vector, full_microbatch)
    if args.only == "alphabet2_complex_vector_r16":
        torch.manual_seed(501)
        complex_vector = AlphabetLM(
            AlphabetLMConfig(
                reader_type="dense_k3",
    ...
    if args.only in {
        ...
        torch.manual_seed(501)
        token_rate = args.only == "alphabet2_token_rate_r16"
        candidate = AlphabetLM(
            AlphabetLMConfig(
                reader_type="dense_k3",
                memory_layout="local_only",
                cnn_pole_memory=True,
                ...
                slow_cnn_pole_coordinate_read=(
                    args.only == "alphabet2_coordinate_read_r16"
                ),
            )
        )
        full_microbatch = torch.randint(32_768, (8, 2_049), device="cuda")
        results[args.only] = _step(candidate, full_microbatch)
    if args.only == "alphabet2_dynamic_transport_r16":
        torch.manual_seed(501)
        dynamic_transport = AlphabetLM(
            AlphabetLMConfig(
                reader_type="dense_k3",
                memory_layout="local_only",
                cnn_pole_memory=True,
        ...
        full_microbatch = torch.randint(32_768, (8, 2_049), device="cuda")
        results[args.only] = _step(dynamic_transport, full_microbatch)
    if args.only == "alphabet2_pole_reader_r16":
        torch.manual_seed(501)
        pole_reader = AlphabetLM(
            AlphabetLMConfig(
                reader_type="dense_k3",
                memory_layout="local_only",
                cnn_pole_memory=True,
        ...
        full_microbatch = torch.randint(32_768, (8, 2_049), device="cuda")
        results[args.only] = _step(pole_reader, full_microbatch)
    if args.only == "all":
        torch.manual_seed(501)
