...
export WANDB_GROUP="A2D-ResAux1-FusionWidth"
...
while [[ ! -f "$output_root/SMOKE_OK" ]]; do
  "$python_bin" -u "$repo/scripts/smoke_a2d_resaux1_f256.py" \
...
while [[ ! -f "$result" ]]; do
  ...
  "$python_bin" -u "$repo/scripts/run_a2d_resaux1_f256_imagenet100.py" \
    --root "$output_root" \
    --data-root "$data_root" \
    --variants "$variant" \
    --run-seeds 501 \
