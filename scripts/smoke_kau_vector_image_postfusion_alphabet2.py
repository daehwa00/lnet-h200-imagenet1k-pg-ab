#!/usr/bin/env python3
"""CUDA train/save/load smoke for the VectorPole image-PostFusion ALPHABET-2."""
...
from lnet.alphabet_lm import LaplaceMambaLMConfig, VectorImagePostFusionAlphabet2LM
...
def main() -> None:
    ...
    model = VectorImagePostFusionAlphabet2LM(config).cuda().train()
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        restored = VectorImagePostFusionAlphabet2LM(config).cuda().eval()
    ...
    if not torch.isfinite(restored_logits).all():
        raise RuntimeError("restored VectorPole image-PostFusion output is non-finite")
    details = f"loss={float(loss.detach()):.6f},shape={tuple(logits.shape)}"
    print(f"VECTOR_IMAGE_POSTFUSION_ALPHABET2_SMOKE={details}", flush=True)
