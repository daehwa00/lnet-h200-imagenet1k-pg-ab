#!/usr/bin/env python3
"""Restart-safe sequential H200 queue for the four XL cells."""
...
SCHEMA = "lnet.h200.imagenet100.k_family_xl.queue.v1"
...
def _runtime() -> dict[str, Any]:
    path = os.environ.get("H200_K_FAMILY_XL_WANDB_RUNTIME")
    if not path:
        raise RuntimeError("H200 XL runtime is missing")
    ...
    if (
        ...
        raise RuntimeError("H200 XL runtime changed")
    return payload
...
def _quarantine_incomplete_result(root: Path, variant: str, seed: int, epochs: int) -> None:
    path = root / "results" / f"{variant}__seed{seed}.json"
    if not path.is_file() or _complete_result(root, variant, seed, epochs):
        return
...
def _load_status(path: Path, runtime: dict[str, Any], target_commit: str) -> dict[str, Any]:
    ...
    if any(payload.get(name) != value for name, value in expected.items()):
        raise RuntimeError("persistent H200 XL queue identity changed")
    if set(payload.get("jobs", {})) != set(VARIANTS):
        raise RuntimeError("persistent H200 XL queue membership changed")
    return payload
...
def main() -> int:
    ...
    if len(target_commit) != 40 or args.workers != 8:
        raise RuntimeError("H200 XL queue requires the frozen commit and exactly 8 workers")
    args.root.mkdir(parents=True, exist_ok=True)
    status_path = args.root / "k-family-xl-queue.json"
    ...
    for variant in VARIANTS:
        ...
        if _complete_result(args.root, variant, training["seed"], training["epochs"]):
            ...
            print(f"H200_K_FAMILY_XL_QUEUE_SKIP={variant}", flush=True)
            continue
        _quarantine_incomplete_result(args.root, variant, training["seed"], training["epochs"])
        job.update(
        ...
        command = [
            sys.executable,
            "scripts/run_h200_imagenet100_k_family_xl.py",
            "--root",
            str(args.root),
            "--data-root",
            str(args.data_root),
        ...
        print(f"H200_K_FAMILY_XL_QUEUE_MODEL={variant}", flush=True)
        try:
            returncode = subprocess.run(command, check=False).returncode
        except Exception as error:
            print(f"H200_XL_QUEUE_LAUNCH_ERROR={variant}:{type(error).__name__}", flush=True)
            returncode = 125
        if Path(os.environ["H200_CONTROL_STOP_MARKER"]).is_file():
            ...
            return 0
        complete = _complete_result(args.root, variant, training["seed"], training["epochs"])
        job.update(
        ...
        if not complete:
            failures += 1
            print(f"H200_K_FAMILY_XL_QUEUE_FAILURE={variant}:{returncode}", flush=True)
        _atomic_json(status_path, status)
    ...
    _atomic_json(status_path, status)
    print("H200_K_FAMILY_XL_QUEUE_STATUS=" + json.dumps(status, sort_keys=True), flush=True)
