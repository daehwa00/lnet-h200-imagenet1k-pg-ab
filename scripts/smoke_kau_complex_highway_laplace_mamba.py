#!/usr/bin/env python3
"""CUDA train/save/load smoke for the Complex-Highway Laplace-Mamba block."""
...
from lnet.alphabet_lm import ComplexHighwayLaplaceMambaLM, LaplaceMambaLMConfig
...
def main() -> None:
    ...
    model = ComplexHighwayLaplaceMambaLM(config).cuda().train()
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        restored = ComplexHighwayLaplaceMambaLM(config).cuda().eval()
    ...
    if not torch.isfinite(restored_logits).all():
        raise RuntimeError("restored Complex-Highway Laplace-Mamba output is non-finite")
    details = f"loss={float(loss.detach()):.6f},shape={tuple(logits.shape)}"
    print(f"COMPLEX_HIGHWAY_LAPLACE_MAMBA_SMOKE={details}", flush=True)
