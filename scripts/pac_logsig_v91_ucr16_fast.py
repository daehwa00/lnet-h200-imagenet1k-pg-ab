"""Fast UCR-16 screen for the structured Legendre-head V9.1."""
...
from optimization.structured_legendre_head import StructuredLegendreHeadALPHABET
from scripts import pac_logsig_v6_ucr16_fast as harness
from scripts import pac_pole_attention_ucr16_fast as base

ROOT = Path(".omx/results/pac-logsig-v91-ucr16-fast-20260721")
VARIANT = "v91_structured_legendre_head"
...
def _source_hashes() -> dict[str, str]:
    ...
    paths = (
        Path("optimization/structured_legendre_head.py"),
        ...
        Path("scripts/pac_logsig_v91_ucr16_fast.py"),
    )
    return {
...
def _design() -> dict[str, object]:
    candidate = StructuredLegendreHeadALPHABET(
        1,
        base.MODEL_DIM,
        base.MODES,
        5,
    ...
    return {
        "schema": "pac_logsig_v91_ucr16_fast_contract.v1",
        "purpose": "validation-only V9.1 screen; no automatic Q1 promotion",
        ...
        "controlled_difference_from_v9": (
            "each 64-channel Legendre coefficient row passes through one shared "
            "64-to-16 linear projection and SiLU before the final linear classifier"
        ),
        "model_contract": {
            ...
            "coefficient_mixer": "shared Linear(64,16) plus SiLU across eight orders",
            "head": "single linear map from 8*16+32+2=162 features",
            "synthesis": False,
            "reader": False,
            "second_scan": False,
...
def jobs() -> list[base.Job]:
    ...
    return [
        base.Job(
            key=f"logsig_v91_ucr16_fast:{dataset}:{VARIANT}:seed{seed}",
            dataset=dataset,
            ...
            estimated_seconds=UCR_SECONDS[dataset] * 0.8,
...
def _build_model(job: base.Job, input_dim: int, output_dim: int):  # noqa: ANN202
    if job.variant != VARIANT:
        message = f"unknown V9.1 fast-screen variant: {job.variant}"
        raise ValueError(message)
    return StructuredLegendreHeadALPHABET(
        input_dim,
        job.model_dim,
        job.modes,
        output_dim,
...
def report(root: Path) -> dict[str, object]:
    ...
    if campaign_status["done"] is not True:
        message = f"refusing to report incomplete V9.1 results: {campaign_status}"
    ...
    if len(rows) != len(expected) or {str(row.get("job_key")) for row in rows} != expected:
        message = "V9.1 completed rows do not match the sealed job set"
        raise RuntimeError(message)
    digest = design_sha256()
    for row in rows:
        if (
            ...
            message = f"invalid, stale, or contaminated V9.1 row: {row.get('job_key')}"
            raise RuntimeError(message)
...
def main() -> None:
    ...
    harness.REPORT_SCHEMA = "pac_logsig_v91_ucr16_fast_report.v1"
    harness.DELTA_FIELD = "v91_minus_h_compact"
