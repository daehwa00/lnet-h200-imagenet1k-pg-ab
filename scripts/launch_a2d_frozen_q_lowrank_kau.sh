...
export WANDB_GROUP="A2D-QRank"

while true; do
  "$python_bin" -u "$repo/scripts/run_a2d_frozen_q_lowrank.py" \
    --cache-root "$cache_root" \
    --output-root "$output_root" \
    --ranks 4 8 16 32 48 64 80 96 100 \
    --head-epochs 30 \
...
complete = payload.get("completed_jobs") == payload.get("expected_jobs") == 30
