...
ROOT="${ROOT:-.omx/results/pac-efp-compact-h-only-ucr18-20260719}"
...
datasets=(
  ...
  FordA FordB GunPoint ItalyPowerDemand MoteStrain Phoneme Plane
...
for dataset in "${datasets[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    if owns_shard "$shard"; then
      ...
      printf '%s %s\n' "$dataset" "$seed" >>"${queue_files[$queue_index]}"
      ((owned_index += 1))
    fi
    ((index += 1))
  done
...
for queue_index in "${!queue_files[@]}"; do
  ...
  xargs -r -n2 -P"$PARALLEL_PER_GPU" bash -c 'run_job "$0" "$@"' "$gpu" <"$queue" &
  workers+=("$!")
done
