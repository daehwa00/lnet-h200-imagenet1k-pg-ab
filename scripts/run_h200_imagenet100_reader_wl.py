#!/usr/bin/env python3
"""Run one strict-versus-WL Reader model with H200 control and relay identities."""
...
def _runtime() -> dict[str, Any]:
    value = os.environ.get("H200_READER_WL_WANDB_RUNTIME")
    if not value:
        raise RuntimeError("H200_READER_WL_WANDB_RUNTIME is required")
    payload = json.loads(Path(value).read_text(encoding="utf-8"))
    if payload.get("schema") != "lnet.h200.imagenet100.reader_wl.runtime.v1":
        raise RuntimeError("invalid H200 strict-versus-WL Reader W&B runtime")
...
def _metadata(variant: str, seed: int) -> dict[str, Any]:
    ...
    if variant not in payload["training"]["variants"] or seed != payload["training"]["seed"]:
        raise RuntimeError(f"unregistered H200 Reader-WL run: {variant}/seed{seed}")
    ...
    if any(os.environ.get(name) != value for name, value in expected.items()):
        raise RuntimeError("H200 Reader-WL W&B environment changed")
...
def _initialize_required_wandb_run(
    ...
    *,
    ...
    try:
        ...
        print(f"H200_READER_WL_WANDB_DEGRADED={type(error).__name__}", flush=True)
        return None
    if run is None or not run.url:
        print("H200_READER_WL_WANDB_DEGRADED=missing_run_url", flush=True)
        return None
...
def _write_heartbeat() -> None:
    _atomic_json(
        ...
        {
            "schema": "lnet.h200.imagenet100.reader_wl.heartbeat.v1",
            "campaign_id": _runtime()["campaign_id"],
            "target_commit": os.environ["H200_EXPECTED_COMMIT"],
...
def _selected_variant() -> str:
    try:
        ...
        raise RuntimeError("H200 Reader-WL worker requires --variants") from error
    values: list[str] = []
    for value in sys.argv[start:]:
        if value.startswith("--"):
            break
        values.append(value)
    if len(values) != 1 or values[0] not in experiment.VARIANTS:
        raise RuntimeError("H200 worker requires exactly one registered Reader variant")
    return values[0]
...
def main() -> None:
    ...
    if initial_stop is not None:
        ...
        return
    print(f"H200_READER_WL_MODEL_START={selected}", flush=True)
    try:
        experiment.main()
