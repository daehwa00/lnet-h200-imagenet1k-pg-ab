"""Fast UCR-16 screen of H-compact with lag-eight moments."""
...
from lnet.pac_h_compact_lag8 import HCompactLag8ALPHABET
...
ROOT = Path(".omx/results/pac-h-compact-lag8-ucr16-fast-20260721")
VARIANT = "h_compact_lags_1_4_8"
...
def _source_hashes() -> dict[str, str]:
    ...
    paths = (
        Path("src/lnet/pac_h_compact_lag8.py"),
        ...
        Path("scripts/pac_h_compact_lag8_ucr16_fast.py"),
    )
    return {
...
def _design() -> dict[str, object]:
    candidate = HCompactLag8ALPHABET(1, base.MODEL_DIM, base.MODES, 5)
    h_contract = harness.H_COMPACT_ROOT / "contract.json"
    h_summary = harness.H_COMPACT_ROOT / "reports/summary.json"
    return {
        "schema": "pac_h_compact_lag8_ucr16_fast_contract.v1",
        "purpose": "validation-only lag-eight addition screen; no automatic Q1 promotion",
        ...
        "controlled_difference": (
            "append normalized complex lag-eight correlations to both writer and "
            "terminal-reader moment banks; initialize only their classifier columns "
            "to zero; retain the canonical H-compact core and linear head"
        ),
        "moment_lags": [1, 4, 8],
        "descriptor_dim": base.MODEL_DIM + 14 * base.MODES,
...
def jobs() -> list[base.Job]:
    ...
    return [
        base.Job(
            key=f"h_compact_lag8_ucr16_fast:{dataset}:{VARIANT}:seed{seed}",
            dataset=dataset,
...
def _build_model(job: base.Job, input_dim: int, output_dim: int):  # noqa: ANN202
    if job.variant != VARIANT:
        message = f"unknown H-compact lag-eight variant: {job.variant}"
        raise ValueError(message)
    return HCompactLag8ALPHABET(
        input_dim,
        job.model_dim,
        job.modes,
        output_dim,
...
def report(root: Path) -> dict[str, object]:
    ...
    if campaign_status["done"] is not True:
        message = f"refusing to report incomplete lag-eight results: {campaign_status}"
    ...
    if len(rows) != len(expected) or {str(row.get("job_key")) for row in rows} != expected:
        message = "lag-eight completed rows do not match the sealed job set"
        raise RuntimeError(message)
    digest = design_sha256()
    for row in rows:
        if (
            ...
            message = f"invalid, stale, or contaminated lag-eight row: {row.get('job_key')}"
            raise RuntimeError(message)
...
def main() -> None:
    ...
    harness.REPORT_SCHEMA = "pac_h_compact_lag8_ucr16_fast_report.v1"
    harness.DELTA_FIELD = "lag8_minus_h_compact"
