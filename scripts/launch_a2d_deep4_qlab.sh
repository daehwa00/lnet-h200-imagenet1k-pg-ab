...
gpu=$5
epochs=$6
runner=run_a2d_resaux1_deep4_imagenet100
variant=A2D-Deep4
...
while [[ ! -f "$output_root/SMOKE_OK" ]]; do
  "$python_bin" -u "$repo/scripts/smoke_a2d_runner.py" \
    --runner "$runner" --device cuda --size 64 --batch-size 2 >>"$log" 2>&1
  status=$?
  if [[ $status -eq 0 ]]; then
...
while [[ ! -f "$result" ]]; do
  ...
  # bf16 storage with fp32 in-kernel accumulation, plus CUDA-graph capture.
  # Validated on A2D-Deep4 over 20 ImageNet-100 epochs (seed 501): accuracy
  # .6534 -> .6542 and affine-only .6394 -> .6420, both inside single-seed
  # noise, at 38m21s -> 27m29s (1.395x).  Neither lever pays alone: graphs
  # cost time while the step is GPU-bound, and only help once bf16 has cut
  # GPU time enough to expose the dispatch gap.
  LNET_COMPILE_MODE=reduce-overhead \
