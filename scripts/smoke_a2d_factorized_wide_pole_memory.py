#!/usr/bin/env python3
"""One real-ImageNet compiled BF16 step for a factorized wide-memory arm."""
...
def _mixed_loss(output: object, targets: Tensor) -> Tensor:
    if not isinstance(output, tuple) or len(output) != 5:
        message = "wide-memory smoke requires the established five-output classifier"
...
def main() -> None:
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        message = "wide-memory smoke requires exactly one visible CUDA device"
        raise RuntimeError(message)
    ...
    stages = [
        module for module in model.modules() if isinstance(module, FactorizedWidePoleMemoryStage)
    ]
    pg_blocks = [
        block
        for stage in stages
        for block in (stage.content_pg, stage.pole_pg, stage.path_pg, stage.post_pg)
    ...
    if (
        ...
        message = f"wide-memory smoke has inactive parameters or blocks: {missing[:8]}"
        raise RuntimeError(message)

    checkpoint = args.root / "factorized-wide-memory-smoke.pt"
    ...
    payload = {
        ...
        "pole_schedule": runner.POLE_SCHEDULES[args.variant],
