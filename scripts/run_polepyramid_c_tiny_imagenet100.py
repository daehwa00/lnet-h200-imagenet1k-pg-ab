...
"""Run the single-seed PolePyramid-C-Tiny ImageNet-100 screen."""
...
from lnet.polepyramid_c_tiny import PolePyramidCTiny, PolePyramidCTinyConfig

if TYPE_CHECKING:
    from argparse import Namespace

VARIANTS = ("polepyramid_c_tiny",)
SEEDS = (501,)


def _build(variant: str, config: PolePyramidCTinyConfig) -> PolePyramidCTiny:
    if variant != "polepyramid_c_tiny":
        message = f"unknown PolePyramid-C-Tiny variant: {variant}"
        raise ValueError(message)
    return PolePyramidCTiny(config)


def _contract(args: Namespace) -> dict[str, Any]:
    config = PolePyramidCTinyConfig()
    data_digest, train_count, validation_count = harness._dataset_digest(args.data_root)
    payload = {
        "schema": "lnet.polepyramid_c_tiny.imagenet100.screening.v1",
        "evidence_status": "exploratory single-seed architecture screen",
        "variants": list(VARIANTS),
        "seeds": list(SEEDS),
        "model": asdict(config),
        "parameter_counts": {
            "polepyramid_c_tiny": sum(
                parameter.numel() for parameter in PolePyramidCTiny().parameters()
            )
        },
        "recipe": {
            "epochs": args.epochs,
        ...
        "source_sha256": {
            ...
            "model": harness._digest(Path("src/lnet/polepyramid_c_tiny.py")),
...
def _summarize(root: Path, contract: dict[str, Any]) -> dict[str, Any] | None:
    path = root / "results" / "polepyramid_c_tiny__seed501.json"
    if not path.exists():
        return None
...
def main() -> None:
    ...
    harness.SEEDS = SEEDS
    harness.ImageNetNanoConfig = PolePyramidCTinyConfig  # type: ignore[attr-defined]
