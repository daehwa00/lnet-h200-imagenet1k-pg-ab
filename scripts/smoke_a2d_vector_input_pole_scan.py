#!/usr/bin/env python3
"""Compile, train, restore, and benchmark the vector-input pole model."""
...
import run_a2d_vector_input_pole_scan_imagenet100 as runner
...
def main() -> None:
    ...
    if not torch.cuda.is_available() or torch.cuda.device_count() != 1:
        message = "vector-input pole smoke requires exactly one visible CUDA device"
    ...
    torch.cuda.manual_seed_all(839)
    torch.set_float32_matmul_precision("high")
    torch.backends.cudnn.benchmark = True

    runner.joint.control.base.base._configure_ramp()
    recipe = control_smoke._recipe(args.batch_size, args.compile_mode)
    ramp = runner.joint.control.base.base.control.control.stemres.uniform.base
    ...
    stages = [
        module for module in model.modules() if isinstance(module, VectorInputPoleMemoryStage)
    ]
    pg_blocks = [
        block
        for stage in stages
        for block in (stage.pole_pg, stage.path_pg)
    ]
    if (
        missing
        or len(stages) != 4
        ...
        message = f"vector-input pole smoke has inactive parameters or blocks: {missing[:8]}"
        raise RuntimeError(message)

    checkpoint = args.root / "vector-input-pole-smoke.pt"
    ...
    payload = {
        ...
        "vector_input_stages": len(stages),
