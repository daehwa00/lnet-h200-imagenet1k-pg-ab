...
seed=501
campaign=${ALPHABET_CAMPAIGN:-/home/daehwa/experiments/alphabet/complex-scan-local-fourier-residual-imagenet100-20260803}
runtime="$campaign/runtime"
python_bin=/home/daehwa/anaconda3/envs/alphabet/bin/python
runner="$runtime/scripts/run_complex_scan_local_fourier_imagenet100.py"
...
if row["parameters"] != contract["parameter_counts"][variant]:
    raise RuntimeError("local Fourier smoke parameter count changed")
if len(row["history"]) != 1:
    raise RuntimeError("local Fourier smoke did not complete exactly one epoch")
values = (
...
if not all(math.isfinite(float(value)) for value in values):
    raise RuntimeError("local Fourier smoke produced a non-finite metric")
PY
...
if [[ ! -f "$campaign/smoke.passed" ]]; then
  echo "local Fourier smoke has not passed" >&2
  exit 1
fi
