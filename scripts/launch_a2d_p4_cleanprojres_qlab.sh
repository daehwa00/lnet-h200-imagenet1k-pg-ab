...
export WANDB_GROUP="${WANDB_GROUP:-A2D-P4-CleanProjRes}"
export WANDB_NAME="${WANDB_NAME:-P4-CleanProjRes}"
...
"$python_bin" -u "$repo/scripts/run_a2d_p4_cleanprojres_imagenet100.py" \
  --root "$root/$variant" \
  --data-root "$data_root" \
  --variants "$variant" \
  --run-seeds "$seed" \
