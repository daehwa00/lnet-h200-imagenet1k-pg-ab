...
if [[ $# -ne 5 ]]; then
  echo "usage: $0 REPO DATA_ROOT OUTPUT_ROOT PYTHON EPOCHS" >&2
  exit 2
fi
...
export WANDB_GROUP="A2D-ResAux1-LinearPath-Confirm100"
...
while [[ ! -f "$output_root/SMOKE_OK" ]]; do
  "$python_bin" -u "$repo/scripts/smoke_a2d_resaux1_linearpath.py" \
...
while [[ ! -f "$result" ]]; do
  ...
  "$python_bin" -u "$repo/scripts/run_a2d_resaux1_linearpath_imagenet100.py" \
    --root "$output_root" \
    --data-root "$data_root" \
    --variants "$variant" \
    --run-seeds 501 \
