"""Fast UCR-16 screen for the residual Legendre-head V9.2."""
...
from optimization.residual_legendre_head import ResidualLegendreHeadALPHABET
from scripts import pac_logsig_v6_ucr16_fast as harness
from scripts import pac_pole_attention_ucr16_fast as base

ROOT = Path(".omx/results/pac-logsig-v92-ucr16-fast-20260721")
VARIANT = "v92_residual_legendre_head"
...
def _source_hashes() -> dict[str, str]:
    ...
    paths = (
        Path("optimization/residual_legendre_head.py"),
        ...
        Path("scripts/pac_logsig_v92_ucr16_fast.py"),
    )
    return {
...
def _design() -> dict[str, object]:
    candidate = ResidualLegendreHeadALPHABET(
        1,
        base.MODEL_DIM,
        base.MODES,
        5,
    ...
    return {
        "schema": "pac_logsig_v92_ucr16_fast_contract.v1",
        "purpose": "validation-only V9.2 screen; no automatic Q1 promotion",
        ...
        "controlled_difference_from_v9": (
            "each 64-channel Legendre coefficient row keeps its original coordinates and "
            "adds one shared 64-to-16-to-64 SiLU residual initialized at scale 0.1"
        ),
        "model_contract": {
            "backbone": "unchanged V9 one-scan full-pole-trajectory encoder",
            "local_readout": "unchanged two-block time-gated pair-pole CNN",
            "temporal_readout": "unchanged degree-8 shifted-Legendre WLS",
            "coefficient_mixer": "shared residual Linear(64,16)-SiLU-Linear(16,64)",
            "coefficient_residual_scale_init": 0.1,
            "head": "unchanged single linear map from 546 features",
            "synthesis": False,
            "reader": False,
            "second_scan": False,
            "terminal_drive": False,
            "log_signature": False,
            "fixed_lags": False,
        },
...
def jobs() -> list[base.Job]:
    ...
    return [
        base.Job(
            key=f"logsig_v92_ucr16_fast:{dataset}:{VARIANT}:seed{seed}",
            dataset=dataset,
            ...
            estimated_seconds=UCR_SECONDS[dataset] * 0.85,
...
def _build_model(job: base.Job, input_dim: int, output_dim: int):  # noqa: ANN202
    if job.variant != VARIANT:
        message = f"unknown V9.2 fast-screen variant: {job.variant}"
        raise ValueError(message)
    return ResidualLegendreHeadALPHABET(
        input_dim,
        job.model_dim,
        job.modes,
        output_dim,
...
def report(root: Path) -> dict[str, object]:
    ...
    if campaign_status["done"] is not True:
        message = f"refusing to report incomplete V9.2 results: {campaign_status}"
    ...
    if len(rows) != len(expected) or {str(row.get("job_key")) for row in rows} != expected:
        message = "V9.2 completed rows do not match the sealed job set"
        raise RuntimeError(message)
    digest = design_sha256()
    for row in rows:
        if (
            ...
            message = f"invalid, stale, or contaminated V9.2 row: {row.get('job_key')}"
            raise RuntimeError(message)
...
def main() -> None:
    ...
    harness.REPORT_SCHEMA = "pac_logsig_v92_ucr16_fast_report.v1"
    harness.DELTA_FIELD = "v92_minus_h_compact"
