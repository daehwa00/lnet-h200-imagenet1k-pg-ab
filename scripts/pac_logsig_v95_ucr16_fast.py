"""Fast UCR-16 screen for state-only physical-time pooling V9.5."""
...
from optimization.state_only_pole_pooling_v95 import StateOnlyPolePoolingALPHABET
from scripts import pac_logsig_v6_ucr16_fast as harness
from scripts import pac_pole_attention_ucr16_fast as base

ROOT = Path(".omx/results/pac-logsig-v95-ucr16-fast-20260721")
VARIANT = "v95_state_only_pole_pooling"
...
def _source_hashes() -> dict[str, str]:
    ...
    paths = (
        ...
        Path("scripts/pac_logsig_v95_ucr16_fast.py"),
    )
    return {
...
def _design() -> dict[str, object]:
    candidate = StateOnlyPolePoolingALPHABET(
        1,
        base.MODEL_DIM,
        base.MODES,
        5,
    ...
    return {
        "schema": "pac_logsig_v95_ucr16_fast_contract.v1",
        "purpose": "validation-only V9.5 screen; no automatic Q1 promotion",
        ...
        "controlled_difference_from_v94": (
            "remove normalized relative time and observation from the relevance scorer, and "
            "remove attended-time and duration descriptors from the linear head"
        ),
        "model_contract": {
            "edge_lift": (
                "finite filled inputs; prefix padding; safe reference interval on padding; "
                "pairwise binary masks before residual causal DWConv1d(k=5,d=1)"
            ),
            ...
            "attention": (
                "state-only per-time Linear(2M,32)-SiLU-Linear(32,1) relevance score; "
                "physical-time base measure; output pools the original 2M pole vector"
            ),
            ...
            "head": "single linear map from 96 features",
...
def jobs() -> list[base.Job]:
    ...
    return [
        base.Job(
            key=f"logsig_v95_ucr16_fast:{dataset}:{VARIANT}:seed{seed}",
            dataset=dataset,
...
def _build_model(job: base.Job, input_dim: int, output_dim: int):  # noqa: ANN202
    if job.variant != VARIANT:
        message = f"unknown V9.5 fast-screen variant: {job.variant}"
        raise ValueError(message)
    return StateOnlyPolePoolingALPHABET(
        input_dim,
        job.model_dim,
        job.modes,
        output_dim,
...
def report(root: Path) -> dict[str, object]:
    ...
    if campaign_status["done"] is not True:
        message = f"refusing to report incomplete V9.5 results: {campaign_status}"
    ...
    if len(rows) != len(expected) or {str(row.get("job_key")) for row in rows} != expected:
        message = "V9.5 completed rows do not match the sealed job set"
        raise RuntimeError(message)
    digest = design_sha256()
    for row in rows:
        if (
            ...
            message = f"invalid, stale, or contaminated V9.5 row: {row.get('job_key')}"
            raise RuntimeError(message)
...
def main() -> None:
    ...
    harness.REPORT_SCHEMA = "pac_logsig_v95_ucr16_fast_report.v1"
    harness.DELTA_FIELD = "v95_minus_h_compact"
