#!/usr/bin/env python3
"""Restart-safe persistent H200 lane for the two canonical K64 P-small-factorial cells."""
...
SCHEMA = "lnet.h200.imagenet100.k64_p_small_factorial.queue.v1"
...
def _runtime() -> dict[str, Any]:
    path = os.environ.get("H200_K64_P_SMALL_FACTORIAL_WANDB_RUNTIME")
    if not path:
        raise RuntimeError("H200 K64 P-small-factorial runtime is missing")
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    if (
        payload.get("schema") != "lnet.h200.imagenet100.k64_p_small_factorial.runtime.v1"
        or tuple(payload.get("training", {}).get("variants", ())) != VARIANTS
    ):
        raise RuntimeError("H200 K64 P-small-factorial runtime changed")
    return payload
...
def main() -> int:
    ...
    status_path = args.root / "k64-p-small-factorial-queue.json"
    ...
    for variant in VARIANTS:
        ...
        if _complete_result(args.root, variant, training["seed"], training["epochs"]):
            ...
            print(f"H200_K64_P_SMALL_FACTORIAL_QUEUE_SKIP={variant}", flush=True)
            continue
        ...
        command = [
            sys.executable,
            "scripts/run_h200_imagenet100_k64_p_small_factorial.py",
            "--root",
            str(args.root),
            "--data-root",
            str(args.data_root),
        ...
        print(f"H200_K64_P_SMALL_FACTORIAL_QUEUE_MODEL={variant}", flush=True)
        try:
        ...
        if not complete:
            failures += 1
            print(f"H200_K64_P_SMALL_FACTORIAL_QUEUE_FAILURE={variant}:{returncode}", flush=True)
        _atomic_json(status_path, status)
    ...
    _atomic_json(status_path, status)
    print("H200_K64_P_SMALL_FACTORIAL_QUEUE_STATUS=" + json.dumps(status, sort_keys=True), flush=True)
