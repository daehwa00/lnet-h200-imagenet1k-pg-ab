#!/usr/bin/env python3
"""CUDA train/save/load smoke for Dense-Write Multi-Observer ALPHABET-2."""
...
from lnet.alphabet_lm import LaplaceMambaLMConfig, MultiObserverImagePostFusionAlphabet2LM
...
def main() -> None:
    ...
    model = MultiObserverImagePostFusionAlphabet2LM(config).cuda().train()
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        restored = MultiObserverImagePostFusionAlphabet2LM(config).cuda().eval()
    ...
    if not torch.isfinite(restored_logits).all():
        raise RuntimeError("restored multi-observer ALPHABET-2 output is non-finite")
    details = f"loss={float(loss.detach()):.6f},shape={tuple(logits.shape)}"
    print(f"MULTI_OBSERVER_ALPHABET2_SMOKE={details}", flush=True)
