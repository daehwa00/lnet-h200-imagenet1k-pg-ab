"""Exercise the K64 P-small-factorial relay with its permanent W&B canary."""
...
def main() -> None:
    runtime = json.loads(
        (ROOT / "h200" / "k64_p_small_factorial" / "campaign.runtime.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    run = wandb.init(
        ...
        config={
            ...
            "purpose": "permanent K64-P-small-factorial relay canary; never experiment evidence",
        ...
        settings=wandb.Settings(
            ...
            x_extra_http_headers={
                "User-Agent": "Mozilla/5.0 lnet-h200-k64-p-small-factorial-canary/1"
            },
    ...
    if "filestream: fatal error" in internal_text:
        ...
        raise RuntimeError(message)
    print(f"H200_K64_P_SMALL_FACTORIAL_RELAY_CANARY_OK={run.url}", flush=True)
