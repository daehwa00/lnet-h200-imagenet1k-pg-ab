"""Exercise the strict-versus-WL Reader relay with its permanent W&B canary."""
...
def main() -> None:
    runtime = json.loads(
        (ROOT / "h200" / "reader_wl" / "campaign.runtime.json").read_text(encoding="utf-8")
    )
    ...
    run = wandb.init(
        ...
        config={
            ...
            "purpose": "permanent Reader-WL relay canary; never experiment evidence",
        ...
        settings=wandb.Settings(
            ...
            x_extra_http_headers={"User-Agent": "Mozilla/5.0 lnet-h200-reader-wl-canary/1"},
    ...
    if "filestream: fatal error" in internal_text:
        ...
        raise RuntimeError(message)
    print(f"H200_READER_WL_RELAY_CANARY_OK={run.url}", flush=True)
