"""Exercise the D2262 P-schedule relay with its permanent W&B canary."""
...
def main() -> None:
    runtime = json.loads(
        (ROOT / "h200" / "d2262_p_schedule" / "campaign.runtime.json").read_text(encoding="utf-8")
    )
    ...
    run = wandb.init(
        ...
        config={
            ...
            "purpose": "permanent D2262 P-schedule relay canary; never experiment evidence",
        ...
        settings=wandb.Settings(
            ...
            x_extra_http_headers={"User-Agent": "Mozilla/5.0 lnet-h200-d2262-p-canary/1"},
    ...
    if "filestream: fatal error" in internal_text:
        ...
        raise RuntimeError(message)
    print(f"H200_D2262_P_RELAY_CANARY_OK={run.url}", flush=True)
