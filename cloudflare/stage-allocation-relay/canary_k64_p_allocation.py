"""Exercise the K64 pole-allocation Reader relay with its permanent W&B canary."""
...
def main() -> None:
    runtime = json.loads(
        (ROOT / "h200" / "k64_p_allocation" / "campaign.runtime.json").read_text(encoding="utf-8")
    )
    ...
    run = wandb.init(
        ...
        config={
            ...
            "purpose": "permanent K64-P-allocation relay canary; never experiment evidence",
        ...
        settings=wandb.Settings(
            ...
            x_extra_http_headers={"User-Agent": "Mozilla/5.0 lnet-h200-k64-p-allocation-canary/1"},
    ...
    if "filestream: fatal error" in internal_text:
        ...
        raise RuntimeError(message)
    print(f"H200_K64_P_ALLOCATION_RELAY_CANARY_OK={run.url}", flush=True)
