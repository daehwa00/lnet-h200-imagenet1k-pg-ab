...
readonly CAMPAIGN_MANIFEST="${PROJECT_ROOT}/h200/reader_wl/campaign.json"
readonly CAMPAIGN_RUNTIME="${PROJECT_ROOT}/h200/reader_wl/campaign.runtime.json"
...
readonly CONTROL_REF="refs/heads/control/imagenet100-reader-wl"
readonly CONTROL_PATH="h200/reader_wl/control.json"
...
if [[ ! -f "${CAMPAIGN_MANIFEST}" || ! -f "${CAMPAIGN_RUNTIME}" ]]; then
  echo "ERROR: generated H200 Reader-WL manifests are missing" >&2
  exit 2
fi
python3 h200/reader_wl/generate_contract.py --check
...
if runtime.get("schema") != "lnet.h200.imagenet100.reader_wl.runtime.v1":
    raise SystemExit("invalid Reader-WL runtime schema")
...
if actual_digest != runtime["campaign_manifest_sha256"]:
    raise SystemExit("Reader-WL campaign digest mismatch")
...
if (
    ...
    raise SystemExit("Reader-WL training matrix changed")
if runtime["program"] != "h200/run_imagenet100_reader_wl.sh":
    raise SystemExit("Reader-WL program identity changed")
...
export H200_READER_WL_WANDB_RUNTIME="${CAMPAIGN_RUNTIME}"
export H200_RUN_CONTROL_REPO_URL="${CONTROL_REPO_URL}"
export H200_RUN_CONTROL_REF="${CONTROL_REF}"
export H200_RUN_CONTROL_PATH="${CONTROL_PATH}"
export H200_RUN_CONTROL_POLL_SECONDS=15
...
timeout --signal=TERM --kill-after=30s 3m \
  "${ENV_ROOT}/bin/python" cloudflare/stage-allocation-relay/canary_reader_wl.py
...
if (( ${#READER_VARIANTS[@]} != 2 )); then
  echo "ERROR: expected exactly 2 strict-versus-WL Reader variants" >&2
  exit 2
fi

for variant in "${READER_VARIANTS[@]}"; do
  echo "H200_READER_WL_QUEUE_MODEL=${variant}"
  timeout --signal=TERM --kill-after=5m 24h \
    "${ENV_ROOT}/bin/python" scripts/run_h200_imagenet100_reader_wl.py \
    --root "${RUN_ROOT}" \
...
if summary.get("completed_runs") != len(variants) or set(summary.get("variants", {})) != set(
    ...
    raise SystemExit("summary does not cover both registered Reader runs")
...
print(
    "H200_READER_WL_RESULTS_COMPLETE="
    + json.dumps(
