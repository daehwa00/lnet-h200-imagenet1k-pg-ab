...
"""Validate and generate the H200 ImageNet-100 strict-versus-WL Reader contract."""
...
MANIFEST_PATH = ROOT / "h200/reader_wl/campaign.json"
...
def _canary(manifest: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "tags": ["H200", "ImageNet-100", "Reader-study", "strict-vs-WL", "relay-canary"],
    }


def _validate(manifest: dict[str, Any], protocol: dict[str, Any], digest: str) -> None:
    if manifest.get("schema") != "lnet.h200.imagenet100.reader_wl.v1":
        raise ValueError("invalid strict-versus-WL Reader campaign schema")
    ...
    if (
        ...
        raise ValueError("strict-versus-WL Reader training matrix changed")
...
def _runtime(manifest: dict[str, Any], digest: str) -> dict[str, Any]:
    return {
        "schema": "lnet.h200.imagenet100.reader_wl.runtime.v1",
        "campaign_id": manifest["campaign_id"],
...
def main() -> int:
    ...
    if args.check:
        ...
        if stale:
            print("stale generated Reader-WL files: " + ", ".join(stale), file=sys.stderr)
            return 1
        return 0
    for path, text in outputs.items():
