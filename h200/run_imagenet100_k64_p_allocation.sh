...
readonly CAMPAIGN_MANIFEST="${PROJECT_ROOT}/h200/k64_p_allocation/campaign.json"
readonly CAMPAIGN_RUNTIME="${PROJECT_ROOT}/h200/k64_p_allocation/campaign.runtime.json"
...
readonly CONTROL_REF="refs/heads/control/imagenet100-k64-p-allocation"
readonly CONTROL_PATH="h200/k64_p_allocation/control.json"
...
if [[ ! -f "${CAMPAIGN_MANIFEST}" || ! -f "${CAMPAIGN_RUNTIME}" ]]; then
  echo "ERROR: generated H200 K64-P-allocation manifests are missing" >&2
  exit 2
fi
python3 h200/k64_p_allocation/generate_contract.py --check
...
if runtime.get("schema") != "lnet.h200.imagenet100.k64_p_allocation.runtime.v1":
    raise SystemExit("invalid K64-P-allocation runtime schema")
...
if actual_digest != runtime["campaign_manifest_sha256"]:
    raise SystemExit("K64-P-allocation campaign digest mismatch")
...
if (
    ...
    raise SystemExit("K64-P-allocation training matrix changed")
if runtime["program"] != "h200/run_imagenet100_k64_p_allocation.sh":
    raise SystemExit("K64-P-allocation program identity changed")
...
export H200_K64_P_ALLOCATION_WANDB_RUNTIME="${CAMPAIGN_RUNTIME}"
export H200_RUN_CONTROL_REPO_URL="${CONTROL_REPO_URL}"
export H200_RUN_CONTROL_REF="${CONTROL_REF}"
export H200_RUN_CONTROL_PATH="${CONTROL_PATH}"
export H200_RUN_CONTROL_POLL_SECONDS=15
...
"${ENV_ROOT}/bin/python" scripts/smoke_h200_k64_p_allocation.py \
  --device cpu \
  --root "${SMOKE_ROOT}/cpu"
timeout --signal=TERM --kill-after=5m 30m \
  ...
  --batch-size 128 \
  --repeat-steps 3
echo "H200_K64_P_ALLOCATION_SMOKE_COMPLETE=${SMOKE_ROOT}"

timeout --signal=TERM --kill-after=30s 3m \
  "${ENV_ROOT}/bin/python" cloudflare/stage-allocation-relay/canary_k64_p_allocation.py
...
mapfile -t K64_P_ALLOCATION_VARIANTS < <(
...
if (( ${#K64_P_ALLOCATION_VARIANTS[@]} != 2 )); then
  echo "ERROR: expected exactly 2 K64 P-allocation variants" >&2
  exit 2
fi

timeout --signal=TERM --kill-after=5m 48h \
  "${ENV_ROOT}/bin/python" scripts/run_h200_k64_p_allocation_queue.py \
  --root "${RUN_ROOT}" \
...
if summary.get("completed_runs") != len(variants) or set(summary.get("variants", {})) != set(
    ...
    raise SystemExit("summary does not cover both registered K64 P-allocation runs")
...
print(
    "H200_K64_P_ALLOCATION_RESULTS_COMPLETE="
    + json.dumps(
