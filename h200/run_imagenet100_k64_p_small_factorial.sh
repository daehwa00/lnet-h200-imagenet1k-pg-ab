...
readonly CAMPAIGN_MANIFEST="${PROJECT_ROOT}/h200/k64_p_small_factorial/campaign.json"
readonly CAMPAIGN_RUNTIME="${PROJECT_ROOT}/h200/k64_p_small_factorial/campaign.runtime.json"
...
readonly CONTROL_REF="refs/heads/control/imagenet100-k64-p-small-factorial"
readonly CONTROL_PATH="h200/k64_p_small_factorial/control.json"
...
if [[ ! -f "${CAMPAIGN_MANIFEST}" || ! -f "${CAMPAIGN_RUNTIME}" ]]; then
  echo "ERROR: generated H200 K64-P-small-factorial manifests are missing" >&2
  exit 2
fi
python3 h200/k64_p_small_factorial/generate_contract.py --check
...
if runtime.get("schema") != "lnet.h200.imagenet100.k64_p_small_factorial.runtime.v1":
    raise SystemExit("invalid K64-P-small-factorial runtime schema")
...
if actual_digest != runtime["campaign_manifest_sha256"]:
    raise SystemExit("K64-P-small-factorial campaign digest mismatch")
...
if (
    ...
    raise SystemExit("K64-P-small-factorial training matrix changed")
if runtime["program"] != "h200/run_imagenet100_k64_p_small_factorial.sh":
    raise SystemExit("K64-P-small-factorial program identity changed")
...
export H200_K64_P_SMALL_FACTORIAL_WANDB_RUNTIME="${CAMPAIGN_RUNTIME}"
export H200_RUN_CONTROL_REPO_URL="${CONTROL_REPO_URL}"
export H200_RUN_CONTROL_REF="${CONTROL_REF}"
export H200_RUN_CONTROL_PATH="${CONTROL_PATH}"
export H200_RUN_CONTROL_POLL_SECONDS=15
...
readonly SMOKE_ROOT="${OUTPUT_BASE}/smoke"
"${ENV_ROOT}/bin/python" scripts/smoke_h200_k64_p_small_factorial.py \
  --device cpu \
  --root "${SMOKE_ROOT}/cpu"
timeout --signal=TERM --kill-after=5m 30m \
  "${ENV_ROOT}/bin/python" scripts/smoke_h200_k64_p_small_factorial.py \
  --device cuda \
  --root "${SMOKE_ROOT}/cuda" \
  --batch-size 128 \
  --repeat-steps 3
echo "H200_K64_P_SMALL_FACTORIAL_SMOKE_COMPLETE=${SMOKE_ROOT}"

timeout --signal=TERM --kill-after=30s 3m \
  "${ENV_ROOT}/bin/python" cloudflare/stage-allocation-relay/canary_k64_p_small_factorial.py
...
mapfile -t K64_P_SMALL_FACTORIAL_VARIANTS < <(
...
if (( ${#K64_P_SMALL_FACTORIAL_VARIANTS[@]} != 2 )); then
  echo "ERROR: expected exactly 2 K64 P-small-factorial variants" >&2
  exit 2
fi

timeout --signal=TERM --kill-after=5m 48h \
  "${ENV_ROOT}/bin/python" scripts/run_h200_k64_p_small_factorial_queue.py \
  --root "${RUN_ROOT}" \
...
if summary.get("completed_runs") != len(variants) or set(summary.get("variants", {})) != set(
    ...
    raise SystemExit("summary does not cover both registered K64 P-small-factorial runs")
...
print(
    "H200_K64_P_SMALL_FACTORIAL_RESULTS_COMPLETE="
    + json.dumps(
