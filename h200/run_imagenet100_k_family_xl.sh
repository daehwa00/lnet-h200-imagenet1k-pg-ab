...
readonly CAMPAIGN_MANIFEST="${PROJECT_ROOT}/h200/k_family_xl/campaign.json"
readonly CAMPAIGN_RUNTIME="${PROJECT_ROOT}/h200/k_family_xl/campaign.runtime.json"
...
readonly CONTROL_REF="refs/heads/control/imagenet100-k-family-xl"
readonly CONTROL_PATH="h200/k_family_xl/control.json"
...
python3 h200/k_family_xl/generate_contract.py --check
...
runtime_path, manifest_path, protocol_path = map(Path, sys.argv[1:])
...
if runtime.get("schema") != "lnet.h200.imagenet100.k_family_xl.runtime.v1":
    raise SystemExit("invalid XL runtime schema")
...
if (
    ...
    or training.get("variants")
    != ["XL-K96-U1", "XL-K96-U125", "XL-K96-Shaped", "XL-K96-Rich"]
):
    raise SystemExit("XL training matrix changed")
if runtime["program"] != "h200/run_imagenet100_k_family_xl.sh":
    raise SystemExit("XL program identity changed")
...
export H200_K_FAMILY_XL_WANDB_RUNTIME="${CAMPAIGN_RUNTIME}"
export H200_RUN_CONTROL_REPO_URL="${CONTROL_REPO_URL}"
export H200_RUN_CONTROL_REF="${CONTROL_REF}"
export H200_RUN_CONTROL_PATH="${CONTROL_PATH}"
export H200_RUN_CONTROL_POLL_SECONDS=15
...
timeout --signal=TERM --kill-after=5m 48h \
  "${ENV_ROOT}/bin/python" scripts/run_h200_k_family_xl_queue.py \
...
print("H200_K_FAMILY_XL_RESULTS_COMPLETE=" + json.dumps({"runs": 4, "epochs": epochs}))
PY
