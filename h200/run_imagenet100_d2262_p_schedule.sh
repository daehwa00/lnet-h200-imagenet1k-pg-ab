...
readonly CAMPAIGN_MANIFEST="${PROJECT_ROOT}/h200/d2262_p_schedule/campaign.json"
readonly CAMPAIGN_RUNTIME="${PROJECT_ROOT}/h200/d2262_p_schedule/campaign.runtime.json"
...
if [[ ! -f "${CAMPAIGN_MANIFEST}" || ! -f "${CAMPAIGN_RUNTIME}" ]]; then
  echo "ERROR: generated H200 D2262 P-schedule manifests are missing" >&2
  exit 2
fi
python3 h200/d2262_p_schedule/generate_contract.py --check
...
if actual_digest != runtime["campaign_manifest_sha256"]:
    raise SystemExit("D2262 P-schedule campaign digest mismatch")
...
if (
    ...
    or len(training.get("variants", [])) != 6
):
    raise SystemExit("D2262 P-schedule training matrix changed")
if runtime["program"] != "h200/run_imagenet100_d2262_p_schedule.sh":
    raise SystemExit("D2262 P-schedule program identity changed")
...
export H200_D2262_P_SCHEDULE_WANDB_RUNTIME="${CAMPAIGN_RUNTIME}"
export H200_RUN_CONTROL_REPO_URL="${CONTROL_REPO_URL}"
export H200_RUN_CONTROL_REF="${CONTROL_REF}"
export H200_RUN_CONTROL_PATH="${CONTROL_PATH}"
export H200_RUN_CONTROL_POLL_SECONDS=15
...
timeout --signal=TERM --kill-after=30s 3m \
  "${ENV_ROOT}/bin/python" cloudflare/stage-allocation-relay/canary_d2262_p_schedule.py
...
if (( ${#P_SCHEDULE_VARIANTS[@]} != 6 )); then
  echo "ERROR: expected exactly 6 D2262 P-schedule variants" >&2
  exit 2
fi

for variant in "${P_SCHEDULE_VARIANTS[@]}"; do
  echo "H200_D2262_P_QUEUE_MODEL=${variant}"
  timeout --signal=TERM --kill-after=5m 24h \
    "${ENV_ROOT}/bin/python" scripts/run_h200_imagenet100_d2262_p_schedule.py \
    --root "${RUN_ROOT}" \
...
if summary.get("completed_runs") != len(variants) or set(summary.get("variants", {})) != set(
    ...
    raise SystemExit("summary does not cover all six registered runs")
