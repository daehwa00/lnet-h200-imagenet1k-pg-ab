...
"""Validate and generate the H200 ImageNet-100 K64 P-small-factorial contract."""
...
MANIFEST_PATH = ROOT / "h200/k64_p_small_factorial/campaign.json"
...
def _canary(manifest: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "tags": ["H200", "ImageNet-100", "K64", "P-small-factorial", "D2262", "relay-canary"],
    }


def _validate(manifest: dict[str, Any], protocol: dict[str, Any], digest: str) -> None:
    if manifest.get("schema") != "lnet.h200.imagenet100.k64_p_small_factorial.v1":
        raise ValueError("invalid K64 P-small-factorial campaign schema")
    ...
    if (
        ...
        raise ValueError("K64 P-small-factorial training matrix changed")
    if (
        ...
        raise ValueError("K64 P-small-factorial campaign identity changed")
    ...
    if (
        ...
        or wandb["group"] != "R2K3-K64-PSmallFactorial-D2262-H200-S501"
...
def main() -> int:
    ...
    if args.check:
        ...
        if stale:
            print(
                "stale generated K64-P-small-factorial files: " + ", ".join(stale),
                file=sys.stderr,
            )
            return 1
        return 0
