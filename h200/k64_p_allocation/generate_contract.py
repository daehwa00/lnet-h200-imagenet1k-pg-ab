...
"""Validate and generate the H200 ImageNet-100 K64 P-allocation contract."""
...
MANIFEST_PATH = ROOT / "h200/k64_p_allocation/campaign.json"
...
def _canary(manifest: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "tags": ["H200", "ImageNet-100", "K64", "P-allocation", "D2262", "relay-canary"],
    }


def _validate(manifest: dict[str, Any], protocol: dict[str, Any], digest: str) -> None:
    if manifest.get("schema") != "lnet.h200.imagenet100.k64_p_allocation.v1":
        raise ValueError("invalid K64 P-allocation campaign schema")
    ...
    if (
        ...
        raise ValueError("K64 P-allocation training matrix changed")
    if (
        ...
        raise ValueError("K64 P-allocation campaign identity changed")
    ...
    if (
        ...
        or wandb["group"] != "R2K3-K64-PAllocation-D2262-H200-S501"
...
def main() -> int:
    ...
    if args.check:
        ...
        if stale:
            print("stale generated K64-P-allocation files: " + ", ".join(stale), file=sys.stderr)
            return 1
        return 0
    for path, text in outputs.items():
