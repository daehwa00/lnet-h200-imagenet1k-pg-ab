...
def write_branch_ablation_artifacts(payload: JsonRow, output_dir: Path) -> tuple[Path, Path]:
    output_dir.mkdir(parents=True, exist_ok=True)
    json_path = output_dir / "hybrid-branch-ablation.json"
    markdown_path = output_dir / "hybrid-branch-ablation.md"
...
def render_markdown(payload: JsonRow) -> str:
    ...
    lines = [
        "# Hybrid Branch Ablation",
        "",
        f"- device: `{payload['device']}`",
        "",
        "## MLP Necessity Conclusion",
        "",
...
def _metadata_lines(config: JsonRow) -> list[str]:
    lines = ["## Controlled Settings", "", "| key | value |", "| --- | --- |"]
