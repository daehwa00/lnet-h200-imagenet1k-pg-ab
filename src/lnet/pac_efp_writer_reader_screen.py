...
from .pac_efp_writer_reader import (
    EFP_WRITER_READER_SCREEN_VARIANTS,
    EFPWriterReaderScreenVariant,
    build_efp_writer_reader_screen_model,
)
...
def run_screen_job(
    ...
    try:
        ...
        model = build_efp_writer_reader_screen_model(config, task.class_count, variant)
        ...
        _atomic_json(
            result_path,
            {
                "schema": "pac_efp_writer_reader_ucr_screen_result.v1",
                "status": "done",
                "dataset": dataset,
                "seed": seed,
                "variant": variant,
                ...
                "config": {
                    ...
                    "weight_decay": 1.0e-4,
                    "grad_clip_norm": 1.0,
                    "writer": "canonical EFP edge stem plus first forward exact-pole block",
                    "reader": (
                        "canonical EFP backward block and head"
                        if variant == "efp16"
                        else "forward read-only exact-pole terminal analyzer"
                    ),
                    "state_evidence": variant,
                    "fusion_initialization": (
                        "identity"
                        if variant == "compact_h_only"
                        else (None if variant == "efp16" else "[identity|zero]")
                    ),
                    "terminal_local_lift_retained": variant != "efp16",
                    "terminal_local_pool_retained": variant != "efp16",
                    "terminal_modal_synthesis": variant == "efp16",
                },
            },
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--variant", choices=EFP_WRITER_READER_SCREEN_VARIANTS, required=True)
