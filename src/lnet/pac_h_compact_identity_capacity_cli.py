"""CLI for the six-capacity identity H-compact screen."""
...
from .pac_h_compact_identity_capacity_campaign import (
    DEFAULT_ROOT,
    enqueue,
    report,
    run_manifest,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Identity H-compact capacity screen")
    parser.add_argument(
        "--stage",
    ...
    parser.add_argument(
        "--trials",
        default="4",
        help="comma-separated confirmatory recipe ids (enqueue only)",
    )
    args = parser.parse_args()
    if args.stage == "preflight":
        payload = synthetic_preflight(args.device)
    elif args.stage == "enqueue":
        trials = tuple(int(value) for value in args.trials.split(",") if value)
        payload = enqueue(args.output_root, trials=trials)
